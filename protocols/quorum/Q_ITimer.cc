...
Array<Q_ITimer*> Q_ITimer::timers;
Q_Time Q_ITimer::min_deadline = Long_max;

Q_ITimer::Q_ITimer(int t, void (*h) ()) {
  state = stopped;
...
void Q_ITimer::handle_timeouts() {
  struct timeval current;
  int ret = gettimeofday(&current, 0);
  th_assert(ret == 0, "gettimeofday PBFT_C_failed");

  for (int i=0; i < timers.size(); i++) {
    Q_ITimer *timer = timers[i];
...
#else
void Q_ITimer::_handle_timeouts(Q_Time current) {
