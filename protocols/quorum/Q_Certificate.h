...
template<class T> class Q_Certificate
{
   //
...
template<class T> inline T *Q_Certificate<T>::cvalue() const
{
   return value;
}

template<class T> inline bool Q_Certificate<T>::is_complete() const
{
   //fprintf(stderr, "gags.count = %d\n", count);
   return count >= complete;
}

template<class T> inline void Q_Certificate<T>::clear()
{
   count = 0;
   bmap.clear();
   value = 0;
...
template<class T> inline void Q_Certificate<T>::set_complete(int c)
{
  complete = c;
}

#endif // Q_Certificate_h
