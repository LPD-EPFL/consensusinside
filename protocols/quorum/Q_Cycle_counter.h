...
static inline long long Q_rdtsc(void) {
  union {
    struct {
  ...
  __asm __volatile (".byte 0xf; .byte 0x31     # Q_rdtsc instruction"
                    : "=a" (v.w32.l), "=d" (v.w32.h) :);
  return v.w64;
}


class Q_Cycle_counter {
public:
  Q_Cycle_counter();
...
private:
  ...
  // after:
  // Q_Cycle_counter c; c.start(); c.stop();
  //
...
inline void Q_Cycle_counter::reset() {
  accumulated = 0;
  running = false;
  max_incr = 0;
}
...
inline Q_Cycle_counter::Q_Cycle_counter() {
  reset();
}


inline void Q_Cycle_counter::start() {
  if (!running) {
    running = true;
    c0 = Q_rdtsc();
  }
...
inline void Q_Cycle_counter::stop() {
  if (running) {
    running = false;
    c1 = Q_rdtsc();
...
inline long long Q_Cycle_counter::elapsed() {
  if (running) {
    c1 = Q_rdtsc();
...
inline long long Q_Cycle_counter::max_increment() {
  return max_incr;
}
