...
#include "PBFT_C_Timer.h"
#include "PBFT_C_Time.h"

#include "Array.h"


PBFT_C_Statistics PBFT_C_stats;

PBFT_C_Statistics::PBFT_C_Statistics() : rec_PBFT_C_stats(20) {
  zero_PBFT_C_stats();
}

void PBFT_C_Statistics::zero_PBFT_C_stats() {
#ifdef PRINT_STATS
  num_gen_auth = 0;
  ...
  if (ret != 0)
    th_fail("getrusage PBFT_C_failed");
...
void PBFT_C_Statistics::print_PBFT_C_stats() {
#ifdef PRINT_STATS
  ...
  for (int i=0; i < rec_counter; i++) {
    printf("Recovery %d: \n", i);
    rec_PBFT_C_stats[i].print_PBFT_C_stats();
  }

  printf("\nPBFT_C_State PBFT_C_stats:\n");
  printf("Num pages fetched = %ld \n", num_fetched);
  printf("Num meta_data fetched = %ld \n", meta_data_fetched);
  printf("PBFT_C_Checkpointing = %qd (cycles) %qd (max. op cycles) ops= %ld \n",
  ...
  printf("PBFT_C_Fetch = %qd (cycles) \n", fetch_cycles.elapsed());

  printf("PBFT_C_Checkpoint Objects Cache: Hits %qd   / Misses %qd\n", cache_hits, cache_misses);

  printf("\nBandwidth PBFT_C_stats:\n");
  printf("Bytes received = %qd Bytes sent = %qd\n", bytes_in, bytes_out);

  printf("\nSyscall PBFT_C_stats:\n");
  ...
  printf("Select = %qd (cycles) %qd (max. op cycles) success = %ld PBFT_C_fail = %ld\n",
         select_cycles.elapsed(), select_cycles.max_increment(), select_success, select_PBFT_C_fail);

  printf("\nHandle timeouts = %qd (cycles) %qd (max. op cycles)\n",
	 handle_timeouts_cycles.elapsed(), handle_timeouts_cycles.max_increment());

  printf("\nPBFT_C_Request retransmissions = %ld\n", req_retrans);

  cycles_after_zero.start();
#endif
}

PBFT_C_Recovery_PBFT_C_stats::PBFT_C_Recovery_PBFT_C_stats() {
  zero_PBFT_C_stats();
}


void PBFT_C_Statistics::init_rec_PBFT_C_stats() {
  rec_counter++;

  rec_PBFT_C_stats._enlarge_by(1);
  PBFT_C_Recovery_PBFT_C_stats& rs = rec_PBFT_C_stats.high();
...
void PBFT_C_Statistics::end_rec_PBFT_C_stats() {
  rec_time.stop();
  PBFT_C_Recovery_PBFT_C_stats& rs = rec_PBFT_C_stats.high();
...
void PBFT_C_Recovery_PBFT_C_stats::zero_PBFT_C_stats() {
  shutdown_time = 0;
  reboot_time = 0;
  restart_time = 0;
  est_time = 0;
...
void PBFT_C_Recovery_PBFT_C_stats::print_PBFT_C_stats() {
  printf("Shutdown = %qd (us)\n", shutdown_time/PBFT_C_clock_mhz);
  printf("Reboot = %qd (us)\n", reboot_time/PBFT_C_clock_mhz);
  printf("Restart = %qd (us)\n", restart_time/PBFT_C_clock_mhz);
  printf("Estimation procedure = %qd (cycles) / %qd (us)\n", est_time, est_time/PBFT_C_clock_mhz);
  printf("Send new key = %qd (us)\n", nk_time/PBFT_C_clock_mhz);
  printf("Send rec. request = %qd (us)\n", rr_time/PBFT_C_clock_mhz);
  printf("Checking = %qd (us) num checked = %qd \n",
	 check_time/PBFT_C_clock_mhz, num_checked);
  printf("PBFT_C_Fetching = %qd (us) num fetches = %qd \n",
	 fetch_time/PBFT_C_clock_mhz, num_fetches);
