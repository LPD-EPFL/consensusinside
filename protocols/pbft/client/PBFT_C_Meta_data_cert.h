#ifndef _PBFT_C_Meta_data_cert_h
#define _PBFT_C_Meta_data_cert_h 1
...
class PBFT_C_Meta_data_cert {
  //
  // A PBFT_C_Meta_data_cert is a set of "matching" meta-data-d messages from
  // different replicas.
  //
  
public:
  PBFT_C_Meta_data_cert();
  // Effects: Creates an empty PBFT_C_Meta_data_cert. 

  ~PBFT_C_Meta_data_cert();
  // Effects: Deletes PBFT_C_Meta_data_cert and all the messages it contains.
 
  bool add(PBFT_C_Meta_data_d *m, bool mine=false);
  // Effects: Adds "m" to the PBFT_C_Meta_data_cert and returns true provided
  // "m" satisfies:
  ...
  void clear();
  // Effects: Discards all messages in PBFT_C_Meta_data_cert
...
private:
  ...
  PBFT_C_Meta_data_d **last_mdds; // Array with the last messages sent by each 
                           // replica.
  class Part_val {
  public:
    Digest d;
...
inline bool PBFT_C_Meta_data_cert::has_mine() const { return has_my_message; }
  
inline bool PBFT_C_Meta_data_cert::cvalue(Seqno& ci, Digest& di) const {
  if (c < 0)
    return false;
...
inline Seqno PBFT_C_Meta_data_cert::last_stable() const { return ls; }

#endif // PBFT_C_Meta_data_cert_h
