#ifndef _PBFT_C_View_info_h
#define _PBFT_C_View_info_h 1
...
class PBFT_C_State;
class PBFT_C_View_change_ack;
class PBFT_C_Status;

class PBFT_C_View_info {
  //
  // Holds information for the view-change protocol.
  //
public:
  PBFT_C_View_info(int id, View v=0);
  ...
  //
  void add_complete(PBFT_C_Pre_prepare* pp);
  ...
  void view_change(View v, Seqno le, PBFT_C_State *state);
  // Requires: All PBFT_C_Pre_prepare messages in complete certificates for views
  ...
  //
  bool add(PBFT_C_View_change* vc);
  ...
  void add(PBFT_C_New_view* nv);
  // Effect: Stores message if it is valid and if it is not obsolete
  // otherwise discards it. This becomes the owner of "nv"'s storage.

  void add(PBFT_C_View_change_ack* vca);
  ...
  PBFT_C_Pre_prepare* fetch_request(Seqno n, Digest &d);
  ...
  void add_missing(PBFT_C_Pre_prepare* pp);
  ...
  void add_missing(PBFT_C_Prepare* p);
...
private:
  ...
  Array<PBFT_C_NV_info> last_nvs;

  PBFT_C_Time vc_sent; // time at which my view-change was last sent
...
inline View PBFT_C_View_info::view() const {
  return v;
}


inline PBFT_C_Pre_prepare* PBFT_C_View_info::fetch_request(Seqno n, Digest &d) {
  return last_nvs[PBFT_C_node->primary(v)].fetch_request(n, d);
}


inline bool PBFT_C_View_info::has_new_view(View vi) const {
  return last_nvs[PBFT_C_node->primary(vi)].complete();
}


inline bool PBFT_C_View_info::has_nv_message(View vi) const {
  if (last_nvs[PBFT_C_node->primary(vi)].view())
    return true;
  return false;
}


inline PBFT_C_View_change_ack* PBFT_C_View_info::my_vc_ack(int id) {
  th_assert(PBFT_C_node->is_replica(id), "Invalid argument");
  return my_vacks[id];
}


inline void PBFT_C_View_info::add_missing(PBFT_C_Pre_prepare* pp) {
  last_nvs[PBFT_C_node->primary(view())].add_missing(pp);
}


inline void  PBFT_C_View_info::add_missing(Digest& rd, Seqno n, int i) {
  last_nvs[PBFT_C_node->primary(view())].add_missing(rd, n, i);
}


inline void PBFT_C_View_info::add_missing(PBFT_C_Prepare* p) {
  last_nvs[PBFT_C_node->primary(view())].add_missing(p);
}


inline PBFT_C_View_info::OReq_info::OReq_info()  {
  m = 0;
  v=-1;
  lv=-1;
  ods = new ODigest_info[PBFT_C_node->f()+2];
}
...
inline void PBFT_C_View_info::OReq_info::clear() {
  delete m;
  m=0;
  v=-1;
  lv=-1;
