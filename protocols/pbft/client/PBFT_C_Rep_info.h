#ifndef _PBFT_C_Rep_info_h
#define _PBFT_C_Rep_info_h 1
...
public:
  ...
  PBFT_C_Rep_info(int nps);
  // Effects: Creates a new object that stores data for "nps"
  // principals.
#else
  PBFT_C_Rep_info(char *mem, int sz, int nps);
...
private:
  int nps;
  char *mem;
  Array<PBFT_C_Reply*> reps; // Array of replies indexed by principal id.
  static const int Max_rep_size = 8192;

  struct Rinfo {
    bool tentative;       // True if last reply is tentative and was not committed.
    PBFT_C_Time lsent;           // PBFT_C_Time at which reply was last sent.
  };
  Array<Rinfo> ireps;
};

inline int PBFT_C_Rep_info::size() const {
  return (nps+1)*Max_rep_size;
}

inline void PBFT_C_Rep_info::commit_reply(int pid) {
  ireps[pid].tentative = false;
  ireps[pid].lsent = zeroPBFT_C_Time();
}

inline bool PBFT_C_Rep_info::is_committed(int pid) {
  return !ireps[pid].tentative;
}

inline Request_id PBFT_C_Rep_info::req_id(int pid) {
  return reps[pid]->request_id();
}

inline Digest& PBFT_C_Rep_info::digest(int pid) {
  return reps[pid]->digest();
}

inline PBFT_C_Reply* PBFT_C_Rep_info::reply(int pid) {
  return reps[pid];
}

#ifndef NO_STATE_TRANSLATION
inline char* PBFT_C_Rep_info::rep_info_mem() {
  return mem;
}
#endif

#endif // _PBFT_C_Rep_info_h
