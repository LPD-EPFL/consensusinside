#ifndef _PBFT_C_New_view_h
#define _PBFT_C_New_view_h 1
...
//
// PBFT_C_New_view messages have the following format:
//
...
struct PBFT_C_New_view_rep : public PBFT_C_Message_rep {
  View v;
...
class PBFT_C_New_view : public PBFT_C_Message {
  //
  //  PBFT_C_New_view messages
  //
public:
  PBFT_C_New_view(View v);
  // Effects: Creates a new (unsigned) PBFT_C_New_view message with an empty
  ...
  static bool convert(PBFT_C_Message *m1, PBFT_C_New_view *&m2);
  // Effects: If "m1" has the right size and tag of a "PBFT_C_New_view",
  // casts "m1" to a "PBFT_C_New_view" pointer, returns the pointer in
  // "m2" and returns true. Otherwise, it returns false.
  // If the conversion is successful it trims excess allocation.

private:
  PBFT_C_New_view_rep& rep() const;
  // Effects: Casts "msg" to a PBFT_C_New_view_rep&
...
inline PBFT_C_New_view_rep& PBFT_C_New_view::rep() const {
  th_assert(ALIGNED(msg), "Improperly aligned pointer");
  return *((PBFT_C_New_view_rep*)msg);
}

inline VC_info* PBFT_C_New_view::vc_info() {
  VC_info* ret = (VC_info *)(contents()+sizeof(PBFT_C_New_view_rep));
  return ret;
}

inline char* PBFT_C_New_view::picked() {
  // Effects: Returns a pointer to the picked array.
  return (char*)(vc_info()+PBFT_C_node->n());
}

inline View PBFT_C_New_view::view() const { return rep().v; }

inline int PBFT_C_New_view::id() const { return PBFT_C_node->primary(view()); }

inline Seqno PBFT_C_New_view::min() const { return rep().min; }

inline Seqno PBFT_C_New_view::max() const { return rep().max; }

inline int PBFT_C_New_view::which_picked(Seqno n) {
  th_assert(n >= min() && n < max(), "Invalid argument");
  return picked()[n-min()];
}

inline bool PBFT_C_New_view::view_change(int id) {
