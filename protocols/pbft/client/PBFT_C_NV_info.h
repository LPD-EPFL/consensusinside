#ifndef _PBFT_C_NV_info_h
#define _PBFT_C_NV_info_h 1
...
public:
  ...
  PBFT_C_View_change *mark_stale(int id);
  ...
  bool add(PBFT_C_New_view* nv, PBFT_C_View_info *parent);
  ...
  bool add(PBFT_C_View_change* vc, bool verified);
  ...
  bool add(PBFT_C_View_change_ack* vca);
...
private:
  ...
  struct VC_info {
    PBFT_C_View_change* vc;
    int ack_count;
    PBFT_C_Bitmap ack_reps;
    bool req_sum;
  ...
  //
  // PBFT_C_Data structures to check the correctness and completness of
  // new-view messages.
  //
  struct Ckpt_sum {
    Seqno n;
  ...
  struct Req_sum {
    ...
    PBFT_C_Bitmap r_pproofs;    // PBFT_C_Replicas that sent positive proofs.
  ...
  void get_proofs(Req_sum& cur, PBFT_C_View_change *vc, Seqno i);
...
inline View PBFT_C_NV_info::view() const {
  return v;
}

inline bool PBFT_C_NV_info::complete() const {
  return is_complete;
}

inline PBFT_C_New_view *PBFT_C_NV_info::new_view(PBFT_C_Time **t) {
  if (t && nv)
    *t = &nv_sent;
  return nv;
}

inline  PBFT_C_View_change *PBFT_C_NV_info::view_change(int id) {
  return vcs[id].vc;
}



#endif // _PBFT_C_NV_info_h
