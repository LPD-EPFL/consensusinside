...
PBFT_C_Meta_data_cert::PBFT_C_Meta_data_cert() {
  last_mdds = new PBFT_C_Meta_data_d*[PBFT_C_node->n()];
  last_stables = new Seqno[PBFT_C_node->n()];
  ...
  max_size = PBFT_C_node->n()*(max_out/checkpoint_interval+1);
...
void PBFT_C_Meta_data_cert::clear() {
  for (int i=0; i < PBFT_C_node->n(); i++) {
    delete last_mdds[i];
    last_mdds[i] = 0;
    last_stables[i] = 0;
...
bool PBFT_C_Meta_data_cert::add(PBFT_C_Meta_data_d* m, bool mine) {
  ...
  if (mine || m->verify()) {
    ...
    if (m->last_stable() > ls)
      ls = K_max<Seqno>(PBFT_C_node->f()+1, last_stables, PBFT_C_node->n(), Seqno_max);
