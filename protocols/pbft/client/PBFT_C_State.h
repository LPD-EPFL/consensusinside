#ifndef _PBFT_C_State_h
#define _PBFT_C_State_h 1
...
class PBFT_C_Fetch;
class PBFT_C_Replica;
class PBFT_C_Meta_data_cert;
...
public:
...
#ifndef NO_STATE_TRANSLATION
  PBFT_C_State(PBFT_C_Replica *replica, int num_objs,
	int (*gets)(int, char **),
  ...
  PBFT_C_State(PBFT_C_Replica *replica, char *memory, int num_bytes);
  ...
  bool restart(FILE* i, PBFT_C_Replica *rep, Seqno ls, Seqno le, bool corrupt); 
  ...
  bool handle(PBFT_C_Fetch *m, Seqno last_stable);
  ...
  bool retrans_fetch(PBFT_C_Time cur) const;
...
private:
  ...
  // PBFT_C_Bitmap with a bit for each block in the memory region indicating
  ...
  Log<PBFT_C_Checkpoint_rec> clog;   // PBFT_C_Checkpoint log
  ...
  PBFT_C_PageCache *pages_lc;        // copies of pages at last checkpoint
#endif
    
  PBFT_C_Meta_data_cert* cert;  // certificate for partition we are working on
  int lreplier;          // id of last replica we chose as replier
  PBFT_C_Time last_fetch_t;     // PBFT_C_Time when last fetch was sent. 
  ...
  bool check_digest(Digest& d, PBFT_C_Meta_data* m);
...
inline bool PBFT_C_State::in_fetch_state() const { return fetching; }

inline bool PBFT_C_State::in_check_state() const { return checking; }

inline bool PBFT_C_State::retrans_fetch(PBFT_C_Time cur) const { 
  return fetching && diffPBFT_C_Time(cur, last_fetch_t) > 100000;
}

#endif // _PBFT_C_State_h
