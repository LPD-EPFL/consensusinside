#ifndef _PBFT_C_Client_h
#define _PBFT_C_Client_h 1
...
class PBFT_C_Client : public PBFT_C_Node {
public:
  PBFT_C_Client(FILE *config_file, FILE *config_priv, short port=0);
  // Effects: Creates a new PBFT_C_Client object using the information in
  ...
  virtual ~PBFT_C_Client();
  // Effects: Deallocates all storage associated with this.

  bool send_request(PBFT_C_Request *req);
...
private:
  PBFT_C_Request *out_req;     // Outstanding request
  ...
  int rtimeout;         // PBFT_C_Timeout period in msecs
  ...
  PBFT_C_Cycle_counter latency; // Used to measure latency.

  // Multiplier used to obtain retransmission timeout from avg_latency
  static const int Rtimeout_mult = 4; 

  PBFT_C_Certificate<PBFT_C_Reply> t_reps; // PBFT_C_Certificate with tentative replies (size 2f+1)
  PBFT_C_Certificate<PBFT_C_Reply> c_reps; // PBFT_C_Certificate with committed replies (size f+1)

  friend void rtimer_handler();
  PBFT_C_ITimer *rtimer;       // Retransmission timer

  void retransmit();
  // Effects: Retransmits any outstanding request and last new-key message.

  void send_new_key();
  // Effects: Calls PBFT_C_Node's send_new_key, and cleans up stale replies in
  // certificates.
};

inline Request_id PBFT_C_Client::get_rid() const { return out_rid; } 

#endif // _PBFT_C_Client_h
