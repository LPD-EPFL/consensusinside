...
PBFT_C_View_change::PBFT_C_View_change(View v, Seqno ls, int id)
  : PBFT_C_Message(PBFT_C_View_change_tag, Max_message_size) {
  rep().v = v;
  rep().ls = ls;
  rep().id = id;

  th_assert(PBFT_C_View_change_rep::prepared_size % 2 == 0, "Invalid max_out");
  ...
  bzero((char*)rep().prepared, PBFT_C_View_change_rep::prepared_size*sizeof(unsigned));
  rep().n_reqs = 0;

  th_assert(ALIGNED(req_info()), "Improperly aligned pointer");
}

void PBFT_C_View_change::add_checkpoint(Seqno n, Digest &d) {
...
void  PBFT_C_View_change::add_request(Seqno n, View v, View lv, Digest &d, bool prepared) {
  th_assert((last_stable() < n) && (n <= last_stable()+max_out), "Invalid argument");
...
bool PBFT_C_View_change::ckpt(Seqno n, Digest &d) {
...
bool PBFT_C_View_change::proofs(Seqno n, View &v, View &lv, Digest &d, bool &prepared) {
  int index = n-last_stable()-1;
...
View PBFT_C_View_change::req(Seqno n, Digest &d) {
...
void PBFT_C_View_change::re_authenticate(PBFT_C_Principal *p) {
  ...
  int old_size = sizeof(PBFT_C_View_change_rep)+sizeof(Req_info)*rep().n_reqs;
...
bool PBFT_C_View_change::verify() {
  int nreqs = rep().n_reqs;
  if (!PBFT_C_node->is_replica(id()) || nreqs < 0 || nreqs > max_out || view() <= 0)
    return false;
  ...
  // Check sizes
  int old_size = sizeof(PBFT_C_View_change_rep)+sizeof(Req_info)*nreqs;
...
bool  PBFT_C_View_change::verify_digest() {
  Digest d =  digest(); // save old digest
  digest().zero();  // zero digest

  bool verified =
    (d == Digest(contents(), sizeof(PBFT_C_View_change_rep)+sizeof(Req_info)*rep().n_reqs));
...
bool PBFT_C_View_change::convert(PBFT_C_Message *m1, PBFT_C_View_change  *&m2) {
  if (!m1->has_tag(PBFT_C_View_change_tag, sizeof(PBFT_C_View_change_rep)))
    return false;

  m1->trim();
  m2 = (PBFT_C_View_change*)m1;
  return true;
}
