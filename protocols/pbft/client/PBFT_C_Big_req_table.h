#ifndef _PBFT_C_Big_req_table_h
#define _PBFT_C_Big_req_table_h 1
...
class PBFT_C_Big_req_table {
  ...
  // whose size is greater than PBFT_C_Request::big_req_thresh.)
  //
public:
  PBFT_C_Big_req_table();
  // Effects: Creates an empty table.

  ~PBFT_C_Big_req_table();
  // Effects: Deletes table and any requests it references.

  void add_pre_prepare(PBFT_C_Request* r, Seqno n, View v);
  ...
  bool add_request(PBFT_C_Request* r, bool verified=true);
  // Requires: r->size() > PBFT_C_Request::big_req_thresh & verified == r->verify() 
  ...
  PBFT_C_Request* lookup(Digest& rd);
...
private:
  bool check_pcerts(BPBFT_C_entry* bre);
  ...
  void remove_unmatched(BPBFT_C_entry* bre);
  // Requires: bre->r != 0
  // Effects: Zeros entry in unmatched.

  Map<Digest,BPBFT_C_entry*> breqs;
  int max_entries;  // Maximum number of entries allowed in the table.
  Array<PBFT_C_Request*> unmatched; // Array of requests that have no waiting pre-prepares
                             // indexed by client id.
};

#endif // _PBFT_C_Big_req_table_h
