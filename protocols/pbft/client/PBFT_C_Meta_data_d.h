#ifndef _PBFT_C_Meta_data_d_h
#define _PBFT_C_Meta_data_d_h 1
...
// 
// PBFT_C_Meta_data_d messages contain the digests of a partition for all the
// checkpoints in the state of the sending replica. They have the
// following format:
//
struct PBFT_C_Meta_data_d_rep : public PBFT_C_Message_rep {
...
class PBFT_C_Meta_data_d : public PBFT_C_Message {
  // 
  //  PBFT_C_Meta_data_d messages
  //
public:
  PBFT_C_Meta_data_d(Request_id r, int l, int i, Seqno ls);
  // Effects: Creates a new un-authenticated PBFT_C_Meta_data_d message with no
  // partition digests.
  ...
  void authenticate(PBFT_C_Principal *p);
  ...
  // Effects: PBFT_C_Fetches the sequence number of last stable checkpoint in message.
  ...
  static bool convert(PBFT_C_Message *m1, PBFT_C_Meta_data_d *&m2);
  // Effects: If "m1" has the right size and tag of a "PBFT_C_Meta_data_d",
  // casts "m1" to a "PBFT_C_Meta_data_d" pointer, returns the pointer in
  // "m2" and returns true. Otherwise, it returns false. Convert also
  // trims any surplus storage from "m1" when the conversion is
  // successfull.
  
private:
  PBFT_C_Meta_data_d_rep &rep() const;
  // Effects: Casts "msg" to a PBFT_C_Meta_data_d_rep&
};

inline PBFT_C_Meta_data_d_rep &PBFT_C_Meta_data_d::rep() const { 
  th_assert(ALIGNED(msg), "Improperly aligned pointer");
  return *((PBFT_C_Meta_data_d_rep*)msg); 
}
 
inline Request_id PBFT_C_Meta_data_d::request_id() const { return rep().rid; }

inline Seqno PBFT_C_Meta_data_d::last_stable() const { return rep().ls; }

inline Seqno PBFT_C_Meta_data_d::last_checkpoint() const { 
  return rep().ls+(rep().n_digests-1)*checkpoint_interval; 
}

inline int PBFT_C_Meta_data_d::num_digests() const { return rep().n_digests; }

inline int PBFT_C_Meta_data_d::level() const { return rep().l; }

inline int PBFT_C_Meta_data_d::index() const { return rep().i; }

inline int PBFT_C_Meta_data_d::id() const { return rep().id; }


#endif // _PBFT_C_Meta_data_d_h
