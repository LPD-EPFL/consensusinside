#ifndef _PBFT_C_Node_h
#define _PBFT_C_Node_h 1
...
class PBFT_C_Message;
class rabin_priv;
class PBFT_C_New_key;
class PBFT_C_ITimer;

extern void PBFT_C_atimer_handler();

class PBFT_C_Node {
public:
  PBFT_C_Node(FILE *config_file, FILE *config_priv, short port=0);
  // Effects: Create a new PBFT_C_Node object using the information in
  ...
  void send(PBFT_C_Message *m, int i);
...
protected:
  ...
  rabin_priv *priv_key; // PBFT_C_Node's private key.

  // Map from principal identifiers to PBFT_C_Principal*. The first "num_replicas"
  // principals correspond to the replicas.
  PBFT_C_Principal **principals;
  int num_principals;

  // Special principal associated with the group of replicas.
  PBFT_C_Principal *group;
  ...
  //
  PBFT_C_ITimer *atimer;
  friend  void PBFT_C_atimer_handler();

  virtual void send_new_key();
  // Effects: Sends a new-key message and updates last_new_key.

  PBFT_C_New_key *last_new_key; // Last new-key message we sent.
...
inline View PBFT_C_Node::view() const { return v; }

inline int PBFT_C_Node::n() const { return num_replicas; }

inline int PBFT_C_Node::f() const { return max_faulty; }

inline int PBFT_C_Node::n_f() const { return threshold; }

inline int PBFT_C_Node::np() const { return num_principals; }

inline int PBFT_C_Node::id() const { return node_id; }

inline PBFT_C_Principal* PBFT_C_Node::i_to_p(int id) const {
  if (id < 0 || id >= num_principals) return 0;
  return principals[id];
}

inline PBFT_C_Principal* PBFT_C_Node::principal() const { return i_to_p(id()); }

inline bool PBFT_C_Node::is_replica(int id) const { return id >= 0 && id <  num_replicas; }

inline int PBFT_C_Node::primary(View vi) const {
  return (vi == v) ? cur_primary : (vi % num_replicas);
}

inline int PBFT_C_Node::primary() const { return cur_primary; }

#ifdef USE_SECRET_SUFFIX_MD5
inline int PBFT_C_Node::auth_size(int id) const {
  if (id < 0) id = node_id;
...
inline void PBFT_C_Node::gen_auth_out(char *src, unsigned src_len, char *dest) const {
  if (dest == 0) dest = src+src_len;
  gen_auth(src, src_len, false, dest);
}

inline void PBFT_C_Node::gen_auth_in(char *src, unsigned src_len, char *dest) const {
  if (dest == 0) dest = src+src_len;
  gen_auth(src, src_len, true, dest);
}

inline bool PBFT_C_Node::verify_auth_in(int i, char *src, unsigned src_len, char *dest) const {
  if (dest == 0) dest = src+src_len;
  return verify_auth(i, src, src_len, true, dest);
}

inline bool PBFT_C_Node::verify_auth_out(int i, char *src, unsigned src_len, char *dest) const {
  if (dest == 0) dest = src+src_len;
  return verify_auth(i, src, src_len, false, dest);
}


inline unsigned PBFT_C_Node::sig_size(int id) const {
  if (id < 0) id = node_id;
...
extern PBFT_C_Node *PBFT_C_node;


#endif // _PBFT_C_Node_h
