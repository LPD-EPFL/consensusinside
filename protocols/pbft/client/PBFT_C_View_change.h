#ifndef _PBFT_C_View_change_h
#define _PBFT_C_View_change_h 1
...
// 
// PBFT_C_View_change messages have the following format:
//
struct PBFT_C_View_change_rep : public PBFT_C_Message_rep {
  ...
  // PBFT_C_Bitmap with bits set for requests that are prepared in req_info
...
class PBFT_C_View_change : public PBFT_C_Message {
  // 
  //  PBFT_C_View_change messages
  //
public:
  PBFT_C_View_change(View v, Seqno ls, int id);
  // Effects: Creates a new (unauthenticated) PBFT_C_View_change message for
  ...
  static bool convert(PBFT_C_Message *m1, PBFT_C_View_change *&m2);
  // Effects: If "m1" has the right size and tag of a "PBFT_C_View_change",
  // casts "m1" to a "PBFT_C_View_change" pointer, returns the pointer in
  // "m2" and returns true. Otherwise, it returns false.
  // If the conversion is successful it trims excess allocation.

private:
  PBFT_C_View_change_rep &rep() const;
  // Effects: Casts "msg" to a PBFT_C_View_change_rep&
...
inline PBFT_C_View_change_rep &PBFT_C_View_change::rep() const { 
  th_assert(ALIGNED(msg), "Improperly aligned pointer");
  return *((PBFT_C_View_change_rep*)msg); 
}
  
inline Req_info *PBFT_C_View_change::req_info() { 
  Req_info *ret = (Req_info *)(contents()+sizeof(PBFT_C_View_change_rep));
  return ret; 
}

inline void PBFT_C_View_change::mark(int index) {
  th_assert(index >= 0 && index < PBFT_C_View_change_rep::prepared_size*INT_BITS, "Out of bounds");
  unsigned *chunk = rep().prepared+index/INT_BITS;
  *chunk |=  (1 << (index%INT_BITS));
}

inline bool PBFT_C_View_change::test(int index) {
  th_assert(index >= 0 && index < PBFT_C_View_change_rep::prepared_size*INT_BITS, "Out of bounds");
  unsigned chunk = *(rep().prepared+index/INT_BITS);
  return (chunk & (1 << (index%INT_BITS))) ? true : false;
}

inline int PBFT_C_View_change::id() const { return rep().id; }

inline  View PBFT_C_View_change::view() const { return rep().v; }

inline  Digest& PBFT_C_View_change::digest() { 
  return rep().d;
}

inline  Seqno PBFT_C_View_change::last_stable() const { return rep().ls; }

inline Seqno PBFT_C_View_change::max_seqno() const { return rep().ls + rep().n_reqs; }

inline  bool PBFT_C_View_change::last_ckpt(Digest& d, Seqno &n) { 
  if (rep().n_ckpts > 0) {
