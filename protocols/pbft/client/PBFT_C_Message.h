#ifndef _PBFT_C_Message_h
#define _PBFT_C_Message_h 1
...
#include "PBFT_C_Log_allocator.h"
...
struct PBFT_C_Message_rep {
  ...
  //Followed by char payload[size-sizeof(PBFT_C_Message_rep)];
};
...
class PBFT_C_Message {
  ...
  //
public:
  PBFT_C_Message(unsigned sz=0);
  // Effects: Creates an untagged PBFT_C_Message object that can hold up
 ...
 ~PBFT_C_Message();
  ...
  // PBFT_C_Pre_prepare.cc

  int size() const;
  // Effects: PBFT_C_Fetches the message size.

  int tag() const;
  // Effects: PBFT_C_Fetches the message tag.
  ...
  bool full() const;
  // Effects: PBFT_C_Messages may be full or empty. Empty messages are just
  // digests of full messages.

  // PBFT_C_Message-specific heap management operators.
...
protected:
  PBFT_C_Message(int t, unsigned sz);
  // Effects: Creates a message with tag "t" that can hold up to "sz"
  // bytes. Useful to create messages to send to the network.

  PBFT_C_Message(PBFT_C_Message_rep *contents);
  // Requires: "contents" contains a valid PBFT_C_Message_rep.
  ...
  int msize() const;
  // Effects: PBFT_C_Fetches the maximum number of bytes that can be stored in
  // this message.

  static bool convert(char *src, unsigned len, int t, int sz, PBFT_C_Message &m);
  // Requires: convert can safely read up to "len" bytes starting at
  // "src" Effects: If "src" is a PBFT_C_Message_rep for which "has_tag(t,
  ...
  friend class PBFT_C_Node;
  friend class PBFT_C_Pre_prepare;

  PBFT_C_Message_rep *msg;    // Pointer to the contents of the message.
...
inline int PBFT_C_Message::size() const {
  return msg->size;
}

inline int PBFT_C_Message::tag() const {
  return msg->tag;
}

inline bool PBFT_C_Message::has_tag(int t, int sz) const {
...
inline  View PBFT_C_Message::view() const { return 0; }

inline bool PBFT_C_Message::full() const { return true; }

inline void* PBFT_C_Message::operator new(size_t s) {
  void *ret = (void*)malloc(ALIGNED_SIZE(s));
  th_assert(ret != 0, "Ran out of memory\n");
  return ret;
}

inline void PBFT_C_Message::operator delete(void *x, size_t s) {
  if (x != 0)
    free(x);
}

inline int PBFT_C_Message::msize() const {
  return (max_size >= 0) ? max_size : msg->size;
}

inline char *PBFT_C_Message::contents() {
  return (char *)msg;
}

#endif //_PBFT_C_Message_h
