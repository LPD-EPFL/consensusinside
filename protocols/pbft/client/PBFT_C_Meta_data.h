#ifndef _PBFT_C_Meta_data_h
#define _PBFT_C_Meta_data_h 1
...
// 
// PBFT_C_Meta_data messages contain information about a partition and its
...
struct PBFT_C_Meta_data_rep : public PBFT_C_Message_rep {
...
class PBFT_C_Meta_data : public PBFT_C_Message {
  // 
  //  PBFT_C_Meta_data messages
  //
public:
  PBFT_C_Meta_data(Request_id r, int l, int i, Seqno lu, Seqno lm, Digest& d);
  // Effects: Creates a new un-authenticated PBFT_C_Meta_data message with no
  ...
  // Effects: PBFT_C_Fetches the last seqno at which partition is up-to-date at sending replica.

  Seqno last_mod() const;
  // Effects: PBFT_C_Fetches seqno of last checkpoint that modified partition.
  ...
  class Sub_parts_iter {
    ...
    // in order.
  public:
    Sub_parts_iter(PBFT_C_Meta_data* m);
    // Requires: PBFT_C_Meta_data is known to be valid
  ...
  static bool convert(PBFT_C_Message *m1, PBFT_C_Meta_data *&m2);
  // Effects: If "m1" has the right size and tag of a "PBFT_C_Meta_data",
  // casts "m1" to a "PBFT_C_Meta_data" pointer, returns the pointer in
  // "m2" and returns true. Otherwise, it returns false. Convert also
  // trims any surplus storage from "m1" when the conversion is
  // successfull.
  
private:
  PBFT_C_Meta_data_rep &rep() const;
  // Effects: Casts "msg" to a PBFT_C_Meta_data_rep&

  Part_info *parts();
};


inline PBFT_C_Meta_data_rep &PBFT_C_Meta_data::rep() const { 
  th_assert(ALIGNED(msg), "Improperly aligned pointer");
  return *((PBFT_C_Meta_data_rep*)msg); 
}
 
inline Part_info *PBFT_C_Meta_data::parts() {
  return (Part_info *)(contents()+sizeof(PBFT_C_Meta_data_rep));
}

inline Request_id PBFT_C_Meta_data::request_id() const { return rep().rid; }

inline Seqno PBFT_C_Meta_data::last_uptodate() const { return rep().lu; }

inline Seqno PBFT_C_Meta_data::last_mod() const { return rep().lm; }

inline int PBFT_C_Meta_data::level() const { return rep().l; }

inline int PBFT_C_Meta_data::index() const { return rep().i; }

inline int PBFT_C_Meta_data::id() const { return rep().id; }

inline Digest& PBFT_C_Meta_data::digest() { return rep().d; }

inline int PBFT_C_Meta_data::num_sparts() const { return rep().np; }

#endif // _PBFT_C_Meta_data_h
