...
template <class T>
PBFT_C_Certificate<T>::PBFT_C_Certificate(int comp) : bmap(Max_num_replicas) {
  max_size = PBFT_C_node->f()+1;
  vals = new PBFT_C_Message_val[max_size];
  cur_size = 0;
  correct = PBFT_C_node->f()+1;
  complete = (comp == 0) ? PBFT_C_node->f()*2+1 : comp;
  c = 0;
  mym = 0;
}

template <class T>
PBFT_C_Certificate<T>::~PBFT_C_Certificate() {
  delete [] vals;
}

template <class T>
bool PBFT_C_Certificate<T>::add(T *m) {
  const int id = m->id();
  if (PBFT_C_node->is_replica(id) && !bmap.test(id)) {
    ...
    if ((c == 0 || (c->count < complete && c->m->match(m))) && m->verify()) {
      ...
      th_assert(id != PBFT_C_node->id(), "verify should return false for messages from self");
...
template <class T>
bool PBFT_C_Certificate<T>::add_mine(T *m) {
  th_assert(m->id() == PBFT_C_node->id(), "Invalid argument");
  th_assert(m->full(), "Invalid argument");

  if (c != 0 && !c->m->match(m)) {
    delete m;
    fprintf(stderr,"PBFT_C_Node is faulty, more than f faulty replicas or faulty primary%s \n", m->stag());
    return false;
  }
...
template <class T>
void PBFT_C_Certificate<T>::mark_stale() {
  if (!is_complete()) {
    int i = 0;
...
template <class T>
T * PBFT_C_Certificate<T>::cvalue_clear() {
  if (c == 0) {
    return 0;
  }
...
template <class T>
bool PBFT_C_Certificate<T>::encode(FILE* o) {
...
template <class T>
bool PBFT_C_Certificate<T>::decode(FILE* in) {
  ...
  for (int i=0; i < cur_size; i++) {
    ...
    if (vals[i].count < 0 || vals[i].count > PBFT_C_node->n())
      return false;

    if (vals[i].count) {
      vals[i].m = (T*)new PBFT_C_Message;
