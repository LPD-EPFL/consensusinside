#ifndef _PBFT_C_Cycle_counter_h
#define _PBFT_C_Cycle_counter_h 1

static inline long long PBFT_C_rdtsc(void) {
  union {
    struct {
...
class PBFT_C_Cycle_counter {
public:
  PBFT_C_Cycle_counter();
...
private:
  ...
  // after:
  // PBFT_C_Cycle_counter c; c.start(); c.stop();
  //
...
inline void PBFT_C_Cycle_counter::reset() {
  accumulated = 0;
  running = false;
  max_incr = 0;
}
...
inline PBFT_C_Cycle_counter::PBFT_C_Cycle_counter() {
  reset();
}


inline void PBFT_C_Cycle_counter::start() {
  if (!running) {
    running = true;
    c0 = PBFT_C_rdtsc();
  }
...
inline void PBFT_C_Cycle_counter::stop() {
  if (running) {
    running = false;
    c1 = PBFT_C_rdtsc();
...
inline long long PBFT_C_Cycle_counter::elapsed() {
  if (running) {
    c1 = PBFT_C_rdtsc();
...
inline long long PBFT_C_Cycle_counter::max_increment() {
  return max_incr;
}

#endif // _PBFT_C_Cycle_counter_h
