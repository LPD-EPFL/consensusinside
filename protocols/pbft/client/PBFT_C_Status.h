#ifndef _PBFT_C_Status_h
#define _PBFT_C_Status_h 1
...
//
// PBFT_C_Status messages have the following format:
//

struct BPBFT_C_info {
  Seqno n;      // Seqno of pre-prepare missing big reqs
  BR_map breqs; // PBFT_C_Bitmap with missing big reqs.
};
...
struct PBFT_C_Status_rep : public PBFT_C_Message_rep {
  View v;                // PBFT_C_Replica's current view
  ...
  if has_nv_info (extra & 1)
    ...
    BPBFT_C_info breqs[brsz]; // missing big requests
  else
...
class PBFT_C_Status : public PBFT_C_Message {
  //
  //  PBFT_C_Status messages
  //
public:
  PBFT_C_Status(View v, Seqno ls, Seqno le, bool hnvi, bool hnvm);
  // Effects: Creates a new unauthenticated PBFT_C_Status message.  "v"
  ...
  static bool convert(PBFT_C_Message *m1, PBFT_C_Status *&m2);
  // Effects: If "m1" has the right size and tag of a "PBFT_C_Status",
  // casts "m1" to a "PBFT_C_Status" pointer, returns the pointer in
  // "m2" and returns true. Otherwise, it returns false.
  // If the conversion is successful it trims excess allocation.

private:
  PBFT_C_Status_rep& rep() const;
  // Effects: Casts "msg" to a PBFT_C_Status_rep&
...
inline PBFT_C_Status_rep& PBFT_C_Status::rep() const {
  th_assert(ALIGNED(msg), "Improperly aligned pointer");
  return *((PBFT_C_Status_rep*)msg);
}


inline char* PBFT_C_Status::prepared() {
  char *ret = contents()+sizeof(PBFT_C_Status_rep);
  return ret;
}


inline char* PBFT_C_Status::committed() {
  char* ret =  prepared()+rep().sz;
  return ret;
}


inline BPBFT_C_info* PBFT_C_Status::breqs() {
  BPBFT_C_info* ret = (BPBFT_C_info*)(committed()+rep().sz);
  return ret;
}


inline char* PBFT_C_Status::vcs() {
  return prepared();
}


inline  PP_info* PBFT_C_Status::pps() {
  PP_info* ret = (PP_info*)(vcs()+PBFT_C_Status_rep::vcs_size);
  return ret;
}


inline void PBFT_C_Status::mark(Seqno n, char *set) {
...
inline bool PBFT_C_Status::test(Seqno n, char *set) {
  int offset = n - last_executed();
  if (offset > 0 && offset <= max_out)
...
inline void PBFT_C_Status::mark_prepared(Seqno n) {
  th_assert(has_nv_info(), "Invalid state");
  mark(n, prepared());
}


inline void  PBFT_C_Status::mark_committed(Seqno n) {
  th_assert(has_nv_info(), "Invalid state");
  mark(n, committed());
}


inline void PBFT_C_Status::add_breqs(Seqno n, BR_map brs) {
  th_assert(has_nv_info(), "Invalid state");
  ...
  BPBFT_C_info& bri = breqs()[rep().brsz++];
  bri.n = n;
  bri.breqs = brs;
}


inline int PBFT_C_Status::id() const { return rep().id; }


inline View PBFT_C_Status::view() const { return rep().v; }


inline bool PBFT_C_Status::has_nv_m() const { return rep().extra & 2; }


inline bool PBFT_C_Status::has_nv_info() const { return rep().extra & 1; }


inline Seqno PBFT_C_Status::last_stable() const { return rep().ls; }


inline Seqno PBFT_C_Status::last_executed() const { return rep().le; }


inline bool PBFT_C_Status::is_prepared(Seqno n) {
  th_assert(has_nv_info(), "Invalid state");
  return test(n, prepared());
}


inline bool PBFT_C_Status::is_committed(Seqno n) {
  th_assert(has_nv_info(), "Invalid state");
  return test(n, committed());
}


inline bool PBFT_C_Status::has_vc(int i) {
  th_assert(!has_nv_info(), "Invalid state");
  th_assert(i >= 0 && i < PBFT_C_Status_rep::vcs_size, "Invalid argument");
  return Bits_test(vcs(), i);
}

#endif // _PBFT_C_Status_h
