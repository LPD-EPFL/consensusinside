...
//
// PBFT_C_PageCache: LRU cache of copies of objects at the last checkpoint taken
...
public:
  ...
  PBFT_C_PageCache() { lru = mru = NULL; parts.predict(MaxElems); }
  // Effects: Creates an empty PBFT_C_PageCache record.
...
inline char* PBFT_C_PageCache::get_obj(int i) {
  BlockCopy *b;
  if (parts.find(i, b))
    return b->data.data;
  else
...
#ifndef NO_STATE_TRANSLATION
PBFT_C_State::PBFT_C_State(PBFT_C_Replica *rep, int num_objs,
	     int (*gets)(int, char **),
...
#ifndef NO_STATE_TRANSLATION
char* PBFT_C_State::get_cached_obj(int i) {
