#ifndef _PBFT_C_Certificate_h
#define _PBFT_C_Certificate_h 1
...
template <class T> class PBFT_C_Certificate {
  //
...
public:
  PBFT_C_Certificate(int complete=0);
  ...
  public:
    Val_iter(PBFT_C_Certificate<T>* c);
...
private:
  PBFT_C_Bitmap bmap; // bitmap with replicas whose message is in this.

  class PBFT_C_Message_val {
  public:
    T *m;
    int count; 
    
    inline PBFT_C_Message_val() { m = 0; count = 0; }
    inline void clear() { 
      delete m; 
      m = 0;
      count = 0;
    }
    inline ~PBFT_C_Message_val() { clear(); }
    
  };
  PBFT_C_Message_val *vals;    // vector with all distinct message values in this
  ...
  PBFT_C_Message_val *c;       // correct certificate value or 0 if unknown.
  ...
  PBFT_C_Time t_sent; // time at which mym was last sent
...
template <class T> 
inline T *PBFT_C_Certificate<T>::mine(PBFT_C_Time **t) { 
  if (t && mym) *t = &t_sent;
  return mym;
}

template <class T> 
inline T *PBFT_C_Certificate<T>::cvalue() const { return (c) ? c->m : 0; }

template <class T> 
inline int PBFT_C_Certificate<T>::num_correct() const {  return (c) ? c->count : 0; }

template <class T>
inline bool PBFT_C_Certificate<T>::is_complete() const { return num_correct() >= complete; }

template <class T>
inline  void PBFT_C_Certificate<T>::make_complete() {
  if (c) {
    c->count = complete;
  }
}
...
template <class T>
inline bool PBFT_C_Certificate<T>::is_empty() const {
  return bmap.all_zero();
}


template <class T>
inline void PBFT_C_Certificate<T>::clear() {
...
template <class T>
inline PBFT_C_Certificate<T>::Val_iter::Val_iter(PBFT_C_Certificate<T>* c) {
  cert = c;
  next = 0;
}
...
template <class T>
inline bool PBFT_C_Certificate<T>::Val_iter::get(T*& m, int& count) {
