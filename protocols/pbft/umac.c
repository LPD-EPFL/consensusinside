/* -----------------------------------------------------------------------
 * 
 * umac.c -- C Implementation UMAC R_Message Authentication
 *
 ...
 * any site outside the United R_States is granted only if you agree 
...
/* Following is the list of UMAC R_parameters supported by this code.       
 * The following R_parameters are fixed in this implementation.             
 *                                                                        
 ...
 * by the above R_parameters, set the following constant to a number
...
/* R_Message "words" are read from memory in an endian-specific manner.     */
...
/* UHASH is a multi-layered algorithm. R_Data presented to UHASH is first
...
static void primitive_verify(void)
{
    ...
    RC6(k1, pt, pt);
    printf("\nRC6 Test\n");
    pbuf(pt, 16, "R_Digest is       ");
    printf("R_Digest should be: %s\n", res);
}
