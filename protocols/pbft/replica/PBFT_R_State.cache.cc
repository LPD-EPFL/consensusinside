...
#include "bhash.t"
#include "buckets.t"
template class Log<PBFT_R_Checkpoint_rec>;
...
// bits.
unsigned long *PBFT_R__Byz_cow_bits = 0;
char *PBFT_R__Byz_mem = 0;
#endif
...
// PBFT_R_Checkpoint record  
class PBFT_R_Checkpoint_rec {
public:
  PBFT_R_Checkpoint_rec();
  ...
  bool is_cleared();
  // Effects: Returns true iff PBFT_R_Checkpoint record is not in use.
  ...
  class Iter {
  public:
    inline Iter(PBFT_R_Checkpoint_rec* r) : g(r->parts) {}
...
inline PBFT_R_Checkpoint_rec::PBFT_R_Checkpoint_rec() : parts(256) {}


inline PBFT_R_Checkpoint_rec::~PBFT_R_Checkpoint_rec() {
  clear();
}


inline void PBFT_R_Checkpoint_rec::append(int l, int i, Part *p) {
  th_assert(!parts.contains(PartKey(l,i)), "Invalid state");
  parts.add(PartKey(l,i), p);
}
  
  
inline void PBFT_R_Checkpoint_rec::appendr(int l, int i, Part *p) {
...
inline Part* PBFT_R_Checkpoint_rec::fetch(int l, int i) {
  Part* p;
...
inline bool PBFT_R_Checkpoint_rec::is_cleared() { 
  return sd.is_zero(); 
}


inline int PBFT_R_Checkpoint_rec::num_entries() const {
  return parts.size();
}


void PBFT_R_Checkpoint_rec::print() {
  printf("PBFT_R_Checkpoint record: %d blocks \n", parts.size()); 
...
void PBFT_R_Checkpoint_rec::clear() {
  if (!is_cleared()) {
    MapGenerator<PartKey,Part*> g(parts);
    PartKey k;
    Part* p;
...
public:
  ...
  void append_oPBFT_R_replace(int l, int i, BlockCopy *b);
  // Effects: Like append but without the requires clause. If fetch(l,
  // i) != 0 it replaces the old mapping, deleting it.
  
  void append_oPBFT_R_replace_without_delete(int l, int i, BlockCopy *b);
  // Effects: Like append but without the requires clause. If fetch(l,
...
inline void Page_mapping::append_oPBFT_R_replace(int l, int i, BlockCopy *b) {
  BlockCopy *tmp;
...
inline void Page_mapping::append_oPBFT_R_replace_without_delete(int l, int i, BlockCopy *b) {
  BlockCopy *tmp;
...
//
// PBFT_R_State methods:
//


#ifndef NO_STATE_TRANSLATION
PBFT_R_State::PBFT_R_State(PBFT_R_Replica *rep, int total_pages,
#ifdef OBJ_REP
	     int (*gets)(int, char **),
...
#endif
  nb(total_pages+PBFT_R_replica->used_state_pages()), 
...
PBFT_R_State::PBFT_R_State(PBFT_R_Replica *rep, char *memory, int num_bytes) : 
  PBFT_R_replica(rep), mem((Block*)memory), nb(num_bytes/Block_size), 
  ...
  rep_mem = new Block[PBFT_R_replica->used_state_pages()];
  for (int i=0; i<PBFT_R_replica->used_state_pages(); i++)
...
#endif
#endif
  cert = new PBFT_R_Meta_data_cert;
  lreplier = 0;
  ...
  // check cow bits.
  PBFT_R__Byz_cow_bits = cowb.bitvec();
  PBFT_R__Byz_mem = (char*)mem;
#endif

}


PBFT_R_State::~PBFT_R_State() {
  for (int i=0; i < PLevels; i++) {
    delete [] ptree[i];
    delete stalep[i];
  }
...
#ifndef APP_CHKPTS
void PBFT_R_State::cow_single(int i) {
#else
void PBFT_R_State::cow_single(int i, bool lib_chkpt) {
#endif
  ...
  bcp = new BlockCopy;
#else
  if (i>=nb-PBFT_R_replica->used_state_pages())
    bcp = new BlockCopy;
#endif
#else
  bcp = new BlockCopy;
  ...
  if (i<nb-PBFT_R_replica->used_state_pages())  // application state
#ifdef APP_CHKPTS
...
#endif
  else                               // PBFT_R_replication library state
    bcp->data = rep_mem[i-nb+PBFT_R_replica->used_state_pages()];

#ifdef APP_CHKPTS
  }
  assert(lib_chkpt||i<nb-PBFT_R_replica->used_state_pages());
#endif
...
void PBFT_R_State::cow(char *m, int size) {
...
void PBFT_R_State::digest(Digest& d, int i, Seqno lm, char *data, int size) {
...
inline int PBFT_R_State::digest(Digest& d, int l, int i) {
  char *data;
  int size;
  ...
  if (l == PLevels-1) {
	...
	//      }
      ...
      data = rep_mem[i-nb+PBFT_R_replica->used_state_pages()].data;
#ifdef OBJ_REP
    size = Block_size;
#endif
    }
  ...
  if (bcp != NULL)
    bcp->d = d;
#else
  if ((l == PLevels-1) && (i < nb-PBFT_R_replica->used_state_pages()))
    delete [] data;
#endif
#endif
...
void PBFT_R_State::compute_full_digest() {
  PBFT_R_Cycle_counter cc;
  cc.start();
  int np = nb;
...
void PBFT_R_State::update_ptree(Seqno n) {
  PBFT_R_Bitmap* mods[PLevels];
  for (int l=0; l < PLevels-1; l++) {
     mods[l] = new PBFT_R_Bitmap(PLevelSize[l]);
  }
  mods[PLevels-1] = &cowb;

  PBFT_R_Checkpoint_rec& cr = clog.fetch(lc);

  for (int l = PLevels-1; l > 0; l--) {
    PBFT_R_Bitmap::Iter iter(mods[l]);
    unsigned int i;
    while (iter.get(i)) {
      Part& p = ptree[l][i];
...
void PBFT_R_State::checkpoint(Seqno seqno) {
  //  printf("PBFT_R_Checkpointing %qd \n", seqno);
  INCPBFT_R_OP(num_ckpts);
  START_CC(ckpt_cycles);

  update_ptree(seqno);

  lc = seqno;
  PBFT_R_Checkpoint_rec &nr = clog.fetch(seqno);
  nr.sd = ptree[0][0].d;
...
Seqno PBFT_R_State::rollback() {
  ...
  while (iter.get(i)) {
    ...
    if ((int)i<nb-PBFT_R_replica->used_state_pages()) {  // application state
      indices[index] = (int)i;
#ifdef OBJ_REP
  ...
  //debug RR-TODO erase XXX
      ...
      rep_mem[i-nb+PBFT_R_replica->used_state_pages()] = b->data;
  }
#ifdef OBJ_REP
...
bool PBFT_R_State::digest(Seqno n, Digest &d) {
  if (!clog.within_range(n)) 
    return false;

  PBFT_R_Checkpoint_rec &rec = clog.fetch(n);
  if (rec.sd.is_zero())
    return false;
...
void PBFT_R_State::discard_checkpoint(Seqno seqno, Seqno le) {
...
//  
#ifdef OBJ_REP
char* PBFT_R_State::get_data(Seqno c, int i, int &objsz) {
#else
char* PBFT_R_State::get_data(Seqno c, int i) {
#endif
  ...
  if (ptree[PLevels-1][i].lm <= c && !cowb.test(i)) {
	...
	delete [] last_obj;
      ...
      objsz = Block_size;
#endif
      return rep_mem[i-nb+PBFT_R_replica->used_state_pages()].data;
    }
    return data;
#endif
  }
  ...
  for (; c <= lc; c += checkpoint_interval) {
    PBFT_R_Checkpoint_rec& r =  clog.fetch(c);
...
Part& PBFT_R_State::get_meta_data(Seqno c, int l, int i) {
...
void PBFT_R_State::start_fetch(Seqno le, Seqno c, Digest *cd, bool stable) {
  ...
  if (!fetching) {
...
#ifndef NO_STATE_TRANSLATION
    th_assert(fetched_pages->num_entries()==0,"PBFT_R_Fetched Pages map not cleared");
#endif
    INCPBFT_R_OP(num_fetches);
    ...
    // system can handle.
    keep_ckpts = false;
    lreplier = lrand48()%PBFT_R_replica->n();
...
void PBFT_R_State::send_fetch(bool change_replier) {
  START_CC(fetch_cycles);

  last_fetch_t = currentPBFT_R_Time();
  Request_id rid = PBFT_R_replica->new_rid();
  //  printf("rid = %qd\n", rid);
  PBFT_R_replica->principal()->set_last_fetch_rid(rid);
  ...
  if (p.c >= 0) {
    ...
    if (change_replier) {
      do {
	lreplier = (lreplier+1)%PBFT_R_replica->n();
      } while (lreplier == PBFT_R_replica->id());
    }
    replier = lreplier;
  }
  ...
  if (checking && ptree[flevel][p.index].lm > check_start) {
    if (flevel == PLevels-1) {
      INCPBFT_R_OP(refetched);
    } else {
      INCPBFT_R_OP(meta_data_refetched);
    }
  }
#endif // PRINT_STATS
  ...
  PBFT_R_Fetch f(rid, p.lu, flevel, p.index, next_chunk, p.c, replier);
#else
  PBFT_R_Fetch f(rid, p.lu, flevel, p.index, p.c, replier);
#endif
  PBFT_R_replica->send(&f, PBFT_R_Node::All_PBFT_R_replicas);
  ...
  if (!cert->has_mine()) {
    ...
    if (!clog.fetch(ls).is_cleared() && p.c <= lc) {
      // Add my PBFT_R_Meta_data_d message to the certificate
      PBFT_R_Meta_data_d* mdd = new PBFT_R_Meta_data_d(rid, flevel, p.index, ls);
...
bool PBFT_R_State::handle(PBFT_R_Fetch *m, Seqno ls) {
  bool verified = true;

  if (m->verify()) {
    PBFT_R_Principal* pi = PBFT_R_replica->i_to_p(m->id());
    int l = m->level();
    int i = m->index();
    ...
    if (pi->last_fetch_rid() < m->request_id() && (l < PLevels-1 || i < nb)) {
      ...
      if (rc >= 0 && m->replier() == PBFT_R_replica->id()) {
	Seqno chosen = -1;
	if (clog.within_range(rc) && !clog.fetch(rc).is_cleared()) {
	  // PBFT_R_Replica has the requested checkpoint
	  chosen = rc;
	} else if (lc >= rc && ptree[l][i].lm <= rc) {
	  // PBFT_R_Replica's last checkpoint has same value as requested
	...
	if (chosen >= 0) {
	  if (l == PLevels-1) {
	    ...
	    if (sz <= m->chunk_number() * Fragment_size) {
	      ...
	      PBFT_R_Data d(i, p.lm, obj, sz, 0);
	      PBFT_R_replica->send(&d, m->id());
	    }
	    else {
	      PBFT_R_Data d(i, p.lm, obj + m->chunk_number() * Fragment_size, sz,
		     m->chunk_number());
	      PBFT_R_replica->send(&d, m->id());
	      //	      fprintf(stderr, "Sending PBFT_R_Data i=%d lm=%qd\n", i, p.lm);
	    }
#else
	    PBFT_R_Data d(i, p.lm, get_data(chosen, i));
	    PBFT_R_replica->send(&d, m->id());
#endif
	    ...
	    // Send meta-data
	    Part& p = get_meta_data(chosen, l, i);
	    PBFT_R_Meta_data md(m->request_id(), l, i, chosen, p.lm, p.d);
	...
	// Send meta-data-d
	PBFT_R_Meta_data_d mdd(m->request_id(), l, i, ls);
...
void PBFT_R_State::handle(PBFT_R_Data *m) {
  INCPBFT_R_OP(num_fetched);
  START_CC(fetch_cycles);
...
#endif
      if (wp.c >= 0 && wp.d == d) {
	INCPBFT_R_OP(num_fetched_a);
	//	fprintf(stderr, "DDDDDPBFT_R_Data i=%d, last chunk=%d, sz=%d\n", i,next_chunk,m->total_size());
...
#endif
	  fetched_pages->append_oPBFT_R_replace(PLevels-1,i,bc);
	  //	  fprintf(stderr,"Added %d\t",i);
	  //	  fetched_pages->print();
	  //	  put_pages(1, &i, &tmp);
	    }
	else                               // PBFT_R_replication library state
	  rep_mem[i-nb+PBFT_R_replica->used_state_pages()] = m->data();
#endif
...
bool PBFT_R_State::check_digest(Digest& d, PBFT_R_Meta_data* m) {
  th_assert(m->level() < PLevels-1, "Invalid argument");

  int l = m->level();
  int i = m->index();
  DSum sum = stree[l][i];
  PBFT_R_Meta_data::Sub_parts_iter miter(m);
  Digest dp;
  int ip;
...
void PBFT_R_State::handle(PBFT_R_Meta_data* m) {
  INCPBFT_R_OP(meta_data_fetched);
  INCPBFT_R_CNT(meta_data_bytes, m->size()); 
  START_CC(fetch_cycles);

  Request_id crid = PBFT_R_replica->principal()->last_fetch_rid();
  ...
  if (fetching && flevel < PLevels-1 && m->request_id() == crid && flevel == m->level()) {
    ...
    if (wp.index == m->index() && wp.c >= 0 && m->digest() == wp.d) {
      // PBFT_R_Requested a specific digest that matches the one in m
      if (m->verify() && check_digest(wp.d, m)) {
	INCPBFT_R_OP(meta_data_fetched_a);
	...
	PBFT_R_Meta_data::Sub_parts_iter iter(m);
	Digest d;
	int index;
...
void PBFT_R_State::handle(PBFT_R_Meta_data_d* m) {
  INCPBFT_R_OP(meta_datad_fetched);
  INCPBFT_R_CNT(meta_datad_bytes, m->size());
  START_CC(fetch_cycles);
  ...
  if (fetching && m->request_id() == crid && flevel == m->level()) {
    ...
    if (wp.index == m->index() && m->last_stable() >= lc && m->last_stable() >= wp.lu) {
      INCPBFT_R_OP(meta_datad_fetched_a);
	...
	if (cert->cvalue(cc, cd)) {
	  // PBFT_R_Certificate is now complete. 
	  wp.c = cc;
	  wp.d = cd;
...
void PBFT_R_State::done_with_level() {
  START_CC(fetch_cycles);
  ...
  if (wp.lu >= wp.lm) {
	...
	printf("\n");*/
      ...
      /*      Page_mapping::Iter itePBFT_R_del(fetched_pages);
      while (itePBFT_R_del.get(ind,lev,b)) {
	delete b;
	} */
	...
	if (clog.within_range(prev) && !clog.fetch(prev).is_cleared()) {
	  PBFT_R_Checkpoint_rec& pr = clog.fetch(prev);
	  PBFT_R_Checkpoint_rec& cr = clog.fetch(lc);
	  PBFT_R_Checkpoint_rec::Iter g(&cr);
	  int pl, pi;
	  Part* p;
	  while (g.get(pl, pi, p)) {
	    pr.appendr(pl, pi, p);
	...
	clog.truncate(lc-max_out);
      ...
      PBFT_R_Checkpoint_rec &nr = clog.fetch(lc);
      nr.sd = ptree[0][0].d;
      cowb.clear();
      stalep[l]->remove();   
      cert->clear();

      PBFT_R_replica->new_state(lc);
    ...
    // PBFT_R_Partition is inconsistent
    if (flevel < refetch_level)
...
//
// PBFT_R_State checking:
//
void PBFT_R_State::start_check(Seqno le) {
  checking = true;
  refetch_level = PLevels;
  lchecked = -1;
  check_start = lc;
...
inline bool PBFT_R_State::check_data(int i) {
...
void PBFT_R_State::check_state() {
  START_CC(check_time);

//  if (refetch_level != 0 && PBFT_R_replica->has_messages(0))
//    return;
  ...
  while (to_check->size() > 0) {    
    ...
    while (lchecked < max) {
      if (count%poll_cnt == 0 && PBFT_R_replica->has_messages(0)) {
	STOP_CC(check_time);
	return;
      }
	...
	INCPBFT_R_OP(num_checked);
	count++;
	lchecked++;
      } else {
	corrupt = true;
	//XXXXXX put these blocks in a queue and fetch them at the end.
	th_fail("PBFT_R_Replica's state is corrupt. Should not happen yet");
      }
    }
  ...
  if (!fetching) {
    ...
    checking = false;
    refetch_level = 0;
    PBFT_R_replica->try_end_recovery();
  }
}


void PBFT_R_State::mark_stale() {
  cert->clear();
}


bool PBFT_R_State::shutdown(FILE* o, Seqno ls) {
  ...
  if (!fetching || keep_ckpts) {
    for (Seqno i=ls; i <= ls+max_out; i++) {
      PBFT_R_Checkpoint_rec& rec = clog.fetch(i);
	...
	PBFT_R_Checkpoint_rec::Iter g(&rec);
	int l,i;
	Part *p;
  ...
  wb += fwrite(rep_mem->data, Block_size, PBFT_R_replica->used_state_pages(), o);
  ab += PBFT_R_replica->used_state_pages();
...
bool PBFT_R_State::restart(FILE* in, PBFT_R_Replica *rep, Seqno ls, Seqno le, bool corrupt) {
  PBFT_R_replica = rep;

#ifndef NO_STATE_TRANSLATION
#ifndef APP_CHKPTS
  for (int i=0; i<nb-PBFT_R_replica->used_state_pages(); i++) {
    if (pages_lc[i])
      delete pages_lc[i];
    pages_lc[i] = NULL;
  }
...
#ifndef NO_STATE_TRANSLATION
  rb += fread(rep_mem->data, Block_size, PBFT_R_replica->used_state_pages(), in);
  ab += PBFT_R_replica->used_state_pages();
#ifdef OBJ_REP
  restart_proc(in);
#endif
...
bool PBFT_R_State::enforce_bound(Seqno b, Seqno ks, bool corrupt) {
  bool ret = true;
  for (int i=0; i < PLevels; i++) {
    int psize = (i != PLevels-1) ? PLevelSize[i] : nb;
