#ifndef _Pre_PBFT_R_Prepare_h
#define _Pre_PBFT_R_Prepare_h 1
...
// 
// PBFT_R_Pre_prepare messages have the following format:
//
struct PBFT_R_Pre_prepare_rep : public PBFT_R_Message_rep {
  View view;       
  Seqno seqno;
...
class PBFT_R_Pre_prepare : public PBFT_R_Message {
  // 
  // PBFT_R_Pre_prepare messages
  //
public:
  PBFT_R_Pre_prepare() : PBFT_R_Message() {}

  PBFT_R_Pre_prepare(View v, Seqno s, PBFT_R_Req_queue &reqs);
  // Effects: Creates a new signed PBFT_R_Pre_prepare message with view
  ...
  PBFT_R_Pre_prepare* clone(View v) const;
  ...
  // the PBFT_R_replica that sent the message if the message is correct.)
  ...
  class PBFT_R_Requests_iter {
    // An iterator for yielding the PBFT_R_Requests in a PBFT_R_Pre_prepare message.
    // Requires: A PBFT_R_Pre_prepare message cannot be modified while it is
    // being iterated on and all the big requests referred to by "m"
    // must be cached.
  public:
    PBFT_R_Requests_iter(PBFT_R_Pre_prepare* m);
    // Requires: PBFT_R_Pre_prepare is known to be valid
    // Effects: Return an iterator for the requests in "m"
	
    bool get(PBFT_R_Request& req);
    // Effects: Updates "req" to "point" to the next request in the
    // PBFT_R_Pre_prepare message and returns true. If there are no more
    // requests, it returns false.

  private:
    PBFT_R_Pre_prepare* msg; 
    char* next_req;
    int big_req;
  };
  friend  class PBFT_R_Requests_iter;
  ...
  // by the PBFT_R_replica "id()", if the digest is correct, and if the
  ...
  // the PBFT_R_replica "id()". If "mode == NRC", it performs all checks
  // except that it does not verify the authenticity of the requests.

  bool check_digest();
  // Effects: Verifies if the digest is correct.

  static bool convert(PBFT_R_Message* m1, PBFT_R_Pre_prepare*& m2);
  // Effects: If "m1" has the right size and tag, casts "m1" to a
  // "PBFT_R_Pre_prepare" pointer, returns the pointer in "m2" and returns
  // true. Otherwise, it returns false. 

private:
  PBFT_R_Pre_prepare_rep& rep() const;
  // Effects: Casts contents to a PBFT_R_Pre_prepare_rep&
...
inline PBFT_R_Pre_prepare_rep& PBFT_R_Pre_prepare::rep() const { 
  th_assert(ALIGNED(msg), "Improperly aligned pointer");
  return *((PBFT_R_Pre_prepare_rep*)msg); 
}

inline char* PBFT_R_Pre_prepare::requests() { 
  char *ret = contents()+sizeof(PBFT_R_Pre_prepare_rep);
  th_assert(ALIGNED(ret), "Improperly aligned pointer");
  return ret; 
}

inline  Digest* PBFT_R_Pre_prepare::big_reqs() {
  char *ret = requests()+rep().rset_size;
  th_assert(ALIGNED(ret), "Improperly aligned pointer");
  return (Digest*)ret;
}

inline char* PBFT_R_Pre_prepare::non_det_choices() {
  char *ret = ((char*)big_reqs())+rep().n_big_reqs*sizeof(Digest);
  th_assert(ALIGNED(ret), "Improperly aligned pointer");
  return ret;
}

inline char* PBFT_R_Pre_prepare::choices(int &len) {
  len = rep().non_det_size;
  return non_det_choices();
}

inline View PBFT_R_Pre_prepare::view() const { return rep().view; }

inline Seqno PBFT_R_Pre_prepare::seqno() const { return rep().seqno; }

inline bool PBFT_R_Pre_prepare::match(const PBFT_R_Prepare* p) const {
  th_assert(view() == p->view() && seqno() == p->seqno(), "Invalid argument");
  return digest() == p->digest();
}

inline Digest& PBFT_R_Pre_prepare::digest() const { return rep().digest; }

inline int PBFT_R_Pre_prepare::num_big_reqs() const { return rep().n_big_reqs; }
 
inline Digest& PBFT_R_Pre_prepare::big_req_digest(int i) {
  th_assert(i >= 0 && i < num_big_reqs(), "Invalid argument");
  return *(big_reqs()+i);
}

#endif // _PBFT_R_Pre_prepare_h
