#ifndef _PBFT_R_Certificate_h
#define _PBFT_R_Certificate_h 1
...
template <class T> class PBFT_R_Certificate {
  //
...
public:
  PBFT_R_Certificate(int complete=0);
  ...
  // from different PBFT_R_replicas. If the complete argument is omitted (or
  ...
  public:
    Val_iter(PBFT_R_Certificate<T>* c);
...
private:
  PBFT_R_Bitmap bmap; // bitmap with PBFT_R_replicas whose message is in this.

  class PBFT_R_Message_val {
  public:
    T *m;
    int count; 
    
    inline PBFT_R_Message_val() { m = 0; count = 0; }
    inline void clear() { 
      delete m; 
      m = 0;
      count = 0;
    }
    inline ~PBFT_R_Message_val() { clear(); }
    
  };
  PBFT_R_Message_val *vals;    // vector with all distinct message values in this
  ...
  PBFT_R_Message_val *c;       // correct certificate value or 0 if unknown.
  ...
  PBFT_R_Time t_sent; // time at which mym was last sent
...
template <class T> 
inline T *PBFT_R_Certificate<T>::mine(PBFT_R_Time **t) { 
  if (t && mym) *t = &t_sent;
  return mym;
}

template <class T> 
inline T *PBFT_R_Certificate<T>::cvalue() const { return (c) ? c->m : 0; }

template <class T> 
inline int PBFT_R_Certificate<T>::num_correct() const {  return (c) ? c->count : 0; }

template <class T>
inline bool PBFT_R_Certificate<T>::is_complete() const { return num_correct() >= complete; }

template <class T>
inline  void PBFT_R_Certificate<T>::make_complete() {
  if (c) {
    c->count = complete;
  }
}
...
template <class T>
inline bool PBFT_R_Certificate<T>::is_empty() const {
  return bmap.all_zero();
}


template <class T>
inline void PBFT_R_Certificate<T>::clear() {
  for (int i=0; i < cuPBFT_R_size; i++) vals[i].clear(); 
  bmap.clear();
  c = 0;
  mym = 0;
  t_sent = 0;
...
template <class T>
inline PBFT_R_Certificate<T>::Val_iter::Val_iter(PBFT_R_Certificate<T>* c) {
  cert = c;
  next = 0;
}
...
template <class T>
inline bool PBFT_R_Certificate<T>::Val_iter::get(T*& m, int& count) {
  if (next < cert->cuPBFT_R_size) {
    m = cert->vals[next].m;
