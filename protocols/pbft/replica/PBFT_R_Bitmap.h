#ifndef _PBFT_R_Bitmap_h
#define _PBFT_R_Bitmap_h 1
...
public:
  PBFT_R_Bitmap(Uint size, bool value=false);
  // Creates a bitmap with all booleans to "value" 

  PBFT_R_Bitmap(PBFT_R_Bitmap const & other);

  ~PBFT_R_Bitmap();

  PBFT_R_Bitmap& operator=(PBFT_R_Bitmap const &other);
  ...
  bool operator== (PBFT_R_Bitmap const other) const;
  ...
  public:
    Iter(PBFT_R_Bitmap *bitmap, bool val=true);
  ...
  private:
    PBFT_R_Bitmap *bitmap;        // The bitmap being yielded. 
...
inline PBFT_R_Bitmap::PBFT_R_Bitmap(Uint sz, bool value) {
  num = sz;
...
inline PBFT_R_Bitmap::PBFT_R_Bitmap(PBFT_R_Bitmap const & other) {
  num = other.num;
  chunks = new Chunk[nc];
  for (Uint i = 0; i < nc; i++) {
...
inline PBFT_R_Bitmap& PBFT_R_Bitmap::operator=(PBFT_R_Bitmap const &other) {
...
inline PBFT_R_Bitmap::~PBFT_R_Bitmap() { delete [] chunks; }

inline Chunk PBFT_R_Bitmap::bitSelector(unsigned int i) const{
  return 1UL << (i%ChunkBits);
}


inline Chunk PBFT_R_Bitmap::valBitSelector(unsigned int i, bool val) const{
  return ((unsigned long)val) << (i%ChunkBits);
}

inline void PBFT_R_Bitmap::clear() {
  for (Uint i = 0; i < nc; i++) chunks[i] = 0UL;
}
    
inline void PBFT_R_Bitmap::setAll() {
  for (Uint i = 0; i < nc; i++) chunks[i] = ~(0UL);
}


inline bool PBFT_R_Bitmap::test(unsigned int i) const {
    th_assert(i < num, "Index out of bounds\n");
    return (chunks[i/ChunkBits] & bitSelector(i)) != 0;
}


inline int PBFT_R_Bitmap::size() const {
    return num;
}

inline void PBFT_R_Bitmap::set(unsigned int i) {
    th_assert(i < num, "Index out of bounds\n");
    chunks[i/ChunkBits] |= bitSelector(i);
}


inline void PBFT_R_Bitmap::assign(unsigned int i, bool val) {
    th_assert(i < num, "Index out of bounds\n");
    chunks[i/ChunkBits] &= (~ bitSelector(i));
...
inline void PBFT_R_Bitmap::reset(unsigned int i) {
    th_assert(i < num, "Index out of bounds\n");
    chunks[i/ChunkBits] &= (~ bitSelector(i));
}

inline void PBFT_R_Bitmap::setRange(Uint min, Uint max) {
  th_assert(min < num && max < num, "Index out of bounds\n");
  for (Uint i = min; i < max; i++) set(i);  
}

inline void PBFT_R_Bitmap::resetRange(Uint min, Uint max) {
  th_assert(min < num && max < num, "Index out of bounds\n");
  for (Uint i = min; i < max; i++) reset(i);  
}

inline void  PBFT_R_Bitmap::print() {
  for (Uint i = 0; i < nc; i++) {
    printf("%lx", chunks[i]);
  }
}
...
inline int PBFT_R_Bitmap::total_set() {
  int retval=0;
...
inline bool PBFT_R_Bitmap::operator== (PBFT_R_Bitmap const other) const {
...
inline bool PBFT_R_Bitmap::all_zero() const {
  for (Uint i = 0; i < nc; i++) 
...
inline PBFT_R_Bitmap::Iter::Iter(PBFT_R_Bitmap *b, bool v):
    bitmap(b), value(v), index(0), ignoreValue((v) ? 0 : ~0){}


inline bool PBFT_R_Bitmap::Iter::get(Uint& ind) {
...
inline bool PBFT_R_Bitmap::encode(FILE* o) {
...
inline bool PBFT_R_Bitmap::decode(FILE* i) {
