...
PBFT_R_View_info::VCA_info::VCA_info() : v(0), vacks((PBFT_R_View_change_ack *)0, PBFT_R_node->n()) {}

void PBFT_R_View_info::VCA_info::clear() {
...
PBFT_R_View_info::PBFT_R_View_info(int ident, View vi) : v(vi), id(ident),
  last_stable(0), oplog(max_out), last_views((View)0, PBFT_R_node->n()),
  last_vcs((PBFT_R_View_change*)0, PBFT_R_node->n()), my_vacks((PBFT_R_View_change_ack*)0, PBFT_R_node->n()), vacks(PBFT_R_node->n()),
  last_nvs(PBFT_R_node->n()) {
    vacks._enlarge_by(PBFT_R_node->n());
    last_nvs._enlarge_by(PBFT_R_node->n());
}


PBFT_R_View_info::~PBFT_R_View_info() {
...
void PBFT_R_View_info::add_complete(PBFT_R_Pre_prepare* pp) {
...
void PBFT_R_View_info::add_incomplete(Seqno n, Digest const &d) {
  OReq_info &ri = oplog.fetch(n);

  if (ri.d == d) {
    // PBFT_R_Message matches the one in the log
    if (ri.m != 0) {
    ...
    // PBFT_R_Message is different from the one in log
    if (ri.m != 0) {
      delete ri.m;
      ri.m = 0;
    } else {
...
void PBFT_R_View_info::send_proofs(Seqno n, View vi, int dest) {
  if (oplog.within_range(n)) {
    OReq_info &ri = oplog.fetch(n);
    PBFT_R_Principal* p = PBFT_R_node->i_to_p(dest);

    for (int i=0; i < PBFT_R_node->f()+2; i++) {
      if (ri.ods[i].v >= vi) {
	PBFT_R_Prepare prep(ri.ods[i].v, n, ri.ods[i].d, p);
	PBFT_R_node->send(&prep, dest);
      }
    }
  }
}
...
PBFT_R_Pre_prepare* PBFT_R_View_info::pre_prepare(Seqno n, Digest& d) {
  if (oplog.within_range(n)) {
    OReq_info& ri = oplog.fetch(n);
    if (ri.m && ri.d == d) {
...
PBFT_R_Pre_prepare* PBFT_R_View_info::pre_prepare(Seqno n, View v) {
  if (oplog.within_range(n)) {
    OReq_info& ri = oplog.fetch(n);
    if (ri.m && ri.v >= v) {
...
bool PBFT_R_View_info::prepare(Seqno n, Digest& d) {
...
void PBFT_R_View_info::discard_old() {
...
void PBFT_R_View_info::view_change(View vi, Seqno last_executed, PBFT_R_State *state) {
  v = vi;

  discard_old();

  // Create my view-change message for "v".
  PBFT_R_View_change *vc = new PBFT_R_View_change(v, last_stable, id);
  ...
  for (Seqno i=last_stable+1; i <= last_stable+max_out; i++) {
    ...
    if (ri.v >= 0) {
      ...
      if (ri.m) {
	// Update PBFT_R_replica's brt to prevent discarding of big requests
...
bool PBFT_R_View_info::add(PBFT_R_View_change* vc) {
  int vci = vc->id();
  int vcv = vc->view();
  ...
  PBFT_R_NV_info &n = last_nvs[PBFT_R_node->primary(vcv)];
  bool stored = false;
  int primv = PBFT_R_node->primary(v);
  if (n.view() == vcv) {
	...
	PBFT_R_View_change_ack *vack = new PBFT_R_View_change_ack(v, id, vci, vc->digest());
...
void PBFT_R_View_info::add(PBFT_R_New_view* nv) {
  int nvi = nv->id();
  int nvv = nv->view();

  if (nvv >= v && nv->verify()) {
    PBFT_R_NV_info &n = last_nvs[nvi];
    if (nv->view() > n.view()) {
...
void PBFT_R_View_info::add(PBFT_R_View_change_ack* vca) {
  int vci = vca->vc_id();
  int vcv = vca->view();

  if (vca->verify()) {
    int primvcv = PBFT_R_node->primary(vcv);

    PBFT_R_NV_info &n = last_nvs[primvcv];
...
inline View PBFT_R_View_info::k_max(int k) const {
  return K_max<View>(k, last_views.as_pointer(), PBFT_R_node->n(), View_max);
}

View PBFT_R_View_info::max_view() const {
  View ret = k_max(PBFT_R_node->f()+1);
  return ret;
}


View PBFT_R_View_info::max_maj_view() const {
  View ret = k_max(PBFT_R_node->n_f());
  return ret;
}

void PBFT_R_View_info::set_received_vcs(PBFT_R_Status *m) {
  th_assert(m->view() == v, "Invalid argument");

  PBFT_R_NV_info& nvi = last_nvs[PBFT_R_node->primary(v)];
  if (nvi.view() == v) {
...
void PBFT_R_View_info::set_missing_pps(PBFT_R_Status *m) {
  th_assert(m->view() == view(), "Invalid argument");

  if (last_nvs[PBFT_R_node->primary(view())].new_view())
    last_nvs[PBFT_R_node->primary(view())].set_missing_pps(m);
}


PBFT_R_View_change *PBFT_R_View_info::my_view_change(PBFT_R_Time **t) {
  PBFT_R_View_change *myvc;
  if (last_vcs[id] == 0) {
    myvc = last_nvs[PBFT_R_node->primary(v)].view_change(id);
  } else {
    myvc = last_vcs[id];
  }
  if (t && myvc)
...
PBFT_R_New_view *PBFT_R_View_info::my_new_view(PBFT_R_Time **t) {
  return last_nvs[id].new_view(t);
}


void PBFT_R_View_info::mark_stable(Seqno ls) {
  last_stable = ls;
  oplog.truncate(last_stable+1);

  last_nvs[PBFT_R_node->primary(v)].mark_stable(ls);
}


void PBFT_R_View_info::clear() {
  ...
  vc_sent = zeroPBFT_R_Time();
}


bool PBFT_R_View_info::shutdown(FILE* o) {
  int wb=0, ab=0;
  bool ret = true;
  for (Seqno i=last_stable+1; i <= last_stable+max_out; i++) {
...
bool PBFT_R_View_info::restart(FILE* in, View rv, Seqno ls, bool corrupt) {
  v = rv;
  last_stable = ls;
  id = PBFT_R_node->id();
  ...
  if (!corrupt) {
    for (Seqno i=last_stable+1; i <= last_stable+max_out; i++) {
      ...
      delete ori.m;
      if (hasm) {
	ori.m = (PBFT_R_Pre_prepare*)new PBFT_R_Message;
	ret &= ori.m->decode(in);
      } else {
	ori.m = 0;
      }
  ...
  ab++;
  if (is_complete)
    last_nvs[PBFT_R_node->primary(v)].make_complete(v);

  return ret & (ab == rb);
}


bool PBFT_R_View_info::enforce_bound(Seqno b, Seqno ks, bool corrupt) {
...
void PBFT_R_View_info::mark_stale() {
  for (int i=0; i < PBFT_R_node->n(); i++) {
    if (i != id) {
      delete last_vcs[i];
      last_vcs[i] = 0;
    ...
    PBFT_R_View_change *vc = last_nvs[i].mark_stale(id);
    if (vc && vc->view() == view()) {
      last_vcs[id] = vc;
    } else {
      delete vc;
