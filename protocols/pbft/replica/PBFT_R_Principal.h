#ifndef _PBFT_R_Principal_h
#define _PBFT_R_Principal_h 1
...
public:
  PBFT_R_Principal(int i, Addr a, char *pkey=0);
  ...
  // Effects: Creates a new PBFT_R_Principal object.
  ...
  bool is_stale(PBFT_R_Time *tv) const;
...
private:
  ...
  PBFT_R_Time my_tstamp;           // my time when message was accepted
...
inline const Addr *PBFT_R_Principal::address() const { 
  return &addr; 
}

inline int PBFT_R_Principal::pid() const { return id;}

inline ULong PBFT_R_Principal::last_tstamp() const { return tstamp; }


inline bool PBFT_R_Principal::is_stale(PBFT_R_Time *tv) const {
  return lessThanPBFT_R_Time(*tv, my_tstamp);
}

inline int PBFT_R_Principal::sig_size() const { return ssize; }

#ifdef USE_SECRET_SUFFIX_MD5
inline bool PBFT_R_Principal::verify_mac_in(const char *src, unsigned src_len, const char *mac) {
  return verify_mac(src, src_len, mac, kin);
}

inline void PBFT_R_Principal::gen_mac_in(const char *src, unsigned src_len, char *dst) {
   gen_mac(src, src_len, dst, kin);
}

inline  bool PBFT_R_Principal::verify_mac_out(const char *src, unsigned src_len, const char *mac) {
  return verify_mac(src, src_len, mac, kout);
}

inline void PBFT_R_Principal::gen_mac_out(const char *src, unsigned src_len, char *dst) {
   gen_mac(src, src_len, dst, kout);
}

inline void PBFT_R_Principal::end_mac(MD5_CTX *context, char *dst, bool in) {
...
inline bool PBFT_R_Principal::verify_mac_in(const char *src, unsigned src_len, 
...
inline void PBFT_R_Principal::gen_mac_in(const char *src, unsigned src_len, char *dst, const char *unonce) {
...
inline  bool PBFT_R_Principal::verify_mac_out(const char *src, unsigned src_len, 
				       const char *mac, const char *unonce) {
...
inline void PBFT_R_Principal::gen_mac_out(const char *src, unsigned src_len, char *dst, const char *unonce) {
...
inline Request_id PBFT_R_Principal::last_fetch_rid() const { return last_fetch; }

inline void  PBFT_R_Principal::set_last_fetch_rid(Request_id r) { last_fetch = r; }
