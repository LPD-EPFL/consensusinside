...
//
// PBFT_R_NV_info::VC_info methods:
//

PBFT_R_NV_info::VC_info::VC_info() : vc(0), ack_count(0), ack_reps(PBFT_R_node->n()), req_sum(false) {}

PBFT_R_NV_info::VC_info::~VC_info() { delete vc; }

void PBFT_R_NV_info::VC_info::clear() {
  delete vc;
  vc = 0;
  ack_count = 0;
  ack_reps.clear();
...
PBFT_R_NV_info::Req_sum::Req_sum() : PBFT_R_pproofs(PBFT_R_node->n()) {}

PBFT_R_NV_info::Req_sum::~Req_sum() { pi.zero(); }
...
//
// PBFT_R_NV_info methods:
//

PBFT_R_NV_info::PBFT_R_NV_info() : v(0), nv(0), vc_target(0), vc_cur(0), vcs(PBFT_R_node->n()) {
  vcs._enlarge_by(PBFT_R_node->n());
  chosen_ckpt = -1;
  max = -1;
  base = -1;
  n_complete = 0;
  is_complete = false;
  nv_sent = zeroPBFT_R_Time();
}


PBFT_R_NV_info::~PBFT_R_NV_info() { clear(); }


void PBFT_R_NV_info::clear() {
  v = 0;
  delete nv;
  nv = 0;
  vc_target = 0;
...
void PBFT_R_NV_info::make_complete(View vi) {
  v = vi;
  is_complete = true;
}


PBFT_R_View_change* PBFT_R_NV_info::mark_stale(int id) {
  PBFT_R_View_change *pres = 0;
  if (!is_complete) {
    pres = vcs[id].vc;
    vcs[id].vc = 0;
    View ov = v;
...
bool PBFT_R_NV_info::add(PBFT_R_New_view* m, PBFT_R_View_info* parent) {
  th_assert(m->verify() || PBFT_R_node->id() == PBFT_R_node->primary(m->view()), "Invalid argument");
...
bool PBFT_R_NV_info::add(PBFT_R_View_change *m, bool verified) {
  ...
  bool is_primary = PBFT_R_node->primary(v) == PBFT_R_node->id();
  ...
  if (!verified) {
    if (is_primary || !m->verify_digest() || vcs[vcid].ack_count < PBFT_R_node->f()) {
      return false;
    }
  }
  ...
  if (is_primary && m->id() == PBFT_R_node->id()) {
...
bool PBFT_R_NV_info::add(PBFT_R_View_change_ack* m) {
  ...
  if (!in_nv && m->vc_digest() == d && !vcs[vci].ack_reps.test(mid)) {
    ...
    if (vcs[vci].ack_count == PBFT_R_node->f()*2-1 && PBFT_R_node->primary(v) == PBFT_R_node->id()) {
      // This view change has enough acks: add it to the new-view.
      PBFT_R_View_change *vc = vcs[vci].vc;
...
void PBFT_R_NV_info::summarize(PBFT_R_View_change *vc) {
  ...
  for (int i=0; i < size; i++) {
    ...
    if (cur.n_proofs >= PBFT_R_node->f()+1 && cur.n_le >= PBFT_R_node->n_f()) {
      choose_ckpt(i);
    }
  }
  ...
  if (!match && !vclc.is_zero()) {
    ...
    if (ns.n_proofs >= PBFT_R_node->f()+1 && ns.n_le >= PBFT_R_node->n_f()) {
...
void PBFT_R_NV_info::choose_ckpt(int index) {
  th_assert(PBFT_R_node->primary(v) == PBFT_R_node->id(), "Invalid state");
  th_assert(index >= 0 && index < ckpts.size(), "Out of bounds");

  Ckpt_sum& cur = ckpts[index];
  th_assert(cur.n_proofs >= PBFT_R_node->f()+1 && cur.n_le >= PBFT_R_node->n_f(), "Invalid argument");
...
void PBFT_R_NV_info::check_comp(Req_sum& cur, Seqno i, int j) {
  th_assert(!is_complete, "Invalid state");

  if (comp_reqs[i-base] < 0 && cur.n_proofs >= PBFT_R_node->f()+1 && cur.n_le >= PBFT_R_node->n_f()) {
    if (!cur.pi.is_complete()) {
      if (cur.pi.pre_prepare() == 0) {
	// Check if the missing pre-prepare is in the log.
	PBFT_R_Pre_prepare* opp = vi->pre_prepare(i, cur.d);

	if (opp) {
	  cur.pi.add(opp->clone(v));
	  cur.n_pproofs = PBFT_R_node->n();
	}
      }
    }

    if (cur.n_pproofs <= PBFT_R_node->f() && vi->prepare(i, cur.d)) {
      // If node sent a prepare for this digest in the past, we do not
      // need more positive proofs.
      cur.n_pproofs = PBFT_R_node->n();
    }

    if (cur.v < 0 || (cur.pi.is_complete() && cur.n_pproofs > PBFT_R_node->f())) {
      comp_reqs[i-base] = j;
      n_complete++;
    } else {
      return;
  ...
  if (n_complete == max-min-1) {
    ...
    if (PBFT_R_replica->primary(v) == PBFT_R_replica->id()) {
      make_new_view();
    } else {
      // Update backups's state to reflect the new view.
      Digest d;
      PBFT_R_View_change* vc = vcs[nv->which_picked(nv->min())].vc;
      Seqno n = vc->last_stable();
      vc->ckpt(n, d);
      th_assert(!d.is_zero(), "Invalid state");
      Seqno ks = known_stable();
      PBFT_R_replica->process_new_view(n, d, nv->max(), ks);
    }
  }
}


Seqno PBFT_R_NV_info::known_stable() {
  th_assert(is_complete, "Invalid state");

  Seqno* maxs =  new Seqno[PBFT_R_node->n()];

  for (int i=0; i < PBFT_R_node->n(); i++)
    maxs[i] = (vcs[i].vc != 0) ? vcs[i].vc->last_stable() : 0;

  Seqno max_stable1 = K_max(PBFT_R_node->f()+1, maxs, PBFT_R_node->n(), Seqno_max);
  ...
  Seqno max_stable2 = K_max(PBFT_R_node->n_f(), maxs, PBFT_R_node->n(), Seqno_max);
...
void PBFT_R_NV_info::get_proofs(Req_sum& cur, PBFT_R_View_change *vc, Seqno i) {
  bool prepared;
  View v, lv;
  Digest d;
...
void PBFT_R_NV_info::summarize_reqs(PBFT_R_View_change *vc) {
...
void PBFT_R_NV_info::make_new_view() {
  th_assert(PBFT_R_node->primary(v) == PBFT_R_node->id(), "Invalid state");
  th_assert(is_complete, "Invalid state");
  th_assert(nv_sent == zeroPBFT_R_Time(), "Invalid state");
  ...
  nv_sent = currentPBFT_R_Time();

  // Update PBFT_R_replica's state to reflect new-view.
  Seqno ks = known_stable();
  PBFT_R_replica->process_new_view(min, ckpts[chosen_ckpt].d, nv->max(), ks);
}


bool PBFT_R_NV_info::check_new_view() {
  th_assert(PBFT_R_node->primary(v) != PBFT_R_node->id(), "Invalid state");

  // Check chosen checkpoint.
  int cid = nv->which_picked(nv->min());
  PBFT_R_View_change* vc = vcs[cid].vc;
  min = vc->last_stable();
  if (min != nv->min())
    return false;
  base = min+1;
  ...
  if (n_proofs < PBFT_R_node->f()+1 || n_le < PBFT_R_node->n_f())
    return false;

  chosen_ckpt = 0;

  // PBFT_R_Checkpoint is correct. Check the value of nv->max(): this value
  ...
  int n_lt = 0;
  for (int i=0; i < vcs.size(); i++) {
    PBFT_R_View_change* vc = vcs[i].vc;
    if (vc == 0)
      continue;
  ...
  if (base == nv->max()) {
    is_complete = true;
    Seqno ks = known_stable();
    PBFT_R_replica->process_new_view(min, d, nv->max(), ks);
    return true;
  }
...
PBFT_R_Pre_prepare* PBFT_R_NV_info::fetch_request(Seqno n, Digest &d) {
  th_assert(is_complete, "Invalid state");
  ...
  if (pv >= 0 && d != null) {
    ...
    pp = new PBFT_R_Pre_prepare(v, n, empty);
    d = pp->digest();
  }

  if (PBFT_R_node->primary(v) == PBFT_R_node->id())
    pp->re_authenticate();
...
void PBFT_R_NV_info::set_received_vcs(PBFT_R_Status *m) {
  if (PBFT_R_node->primary(v) != PBFT_R_node->id()) {
    // Not primary.
    Digest d;
    for (int i=0; i < PBFT_R_node->n(); i++) {
...
void PBFT_R_NV_info::set_missing_pps(PBFT_R_Status *m) {
  for (Seqno i=base; i < max; i++) {
    ...
    for (int j=0; j < reqsi.size(); j++) {
      Req_sum& cur = reqsi[j];
      if (cur.v >= 0 && cur.v < vpp && cur.n_proofs >= PBFT_R_node->f()+1 && cur.n_le >= PBFT_R_node->n_f()) {
	vpp = cur.v;

	if (cur.n_pproofs <= PBFT_R_node->f())
	  need_proofs = true;
...
void PBFT_R_NV_info::add_missing(PBFT_R_Pre_prepare* pp) {
  Seqno ppn = pp->seqno();
...
void PBFT_R_NV_info::add_missing(Digest& rd, Seqno ppn, int i) {
  if (chosen_ckpt >= 0 && ppn > min && ppn < max && comp_reqs[ppn-base] < 0) {
    Array<Req_sum>& reqspp = reqs[ppn-base];
...
void PBFT_R_NV_info::add_missing(PBFT_R_Prepare* p) {
  Seqno pn = p->seqno();
  ...
  if (chosen_ckpt >= 0 && pn > min && pn < max && comp_reqs[pn-base] < 0) {
    ...
    for (int j=0; j < reqsp.size(); j++) {
      ...
      if (cur.d == p->digest()) {
	if (cur.n_pproofs <= PBFT_R_node->f() && !cur.PBFT_R_pproofs.test(p->id())) {
	  cur.n_pproofs++;
	  cur.PBFT_R_pproofs.set(p->id());
	  check_comp(cur, pn, j);
	}
	break;
      }
...
void PBFT_R_NV_info::mark_stable(Seqno ls) {
  if (v > 0 && !is_complete && chosen_ckpt >= 0 && ls >= max && PBFT_R_node->primary(v) != PBFT_R_node->id()) {
    // If I am not the primary, I can use the fact that ls is stable
    // to trim the number of pre-prepares I need proofs for.
    is_complete = true;
    PBFT_R_replica->process_new_view(ls, Digest(), ls, ls);
  }
}
