...
#include "PBFT_R_Timer.h"
#include "PBFT_R_Time.h"

#include "Array.h"


PBFT_R_Statistics stats;

PBFT_R_Statistics::PBFT_R_Statistics() : rec_stats(20) {
  zero_stats();
}

void PBFT_R_Statistics::zero_stats() {
#ifdef PRINT_STATS
  num_gen_auth = 0;
  gen_auth_cycles.reset();
  num_vePBFT_R_auth = 0;
  vePBFT_R_auth_cycles.reset();
  reply_auth = 0;
  reply_auth_cycles.reset(); 
  reply_auth_ver = 0;
  reply_auth_vePBFT_R_cycles.reset();
  ...
  cycles_aftePBFT_R_zero.reset();
  cycles_aftePBFT_R_zero.start();
#endif
}
...
void PBFT_R_Statistics::print_stats() {
#ifdef PRINT_STATS
  cycles_aftePBFT_R_zero.stop();
  ...
  printf("\nCycles in this interval: %qd \n", cycles_aftePBFT_R_zero.elapsed());
  ...
  printf("Verify = %qd (cycles) %qd (max. op cycles) ops= %ld \n", 
	 vePBFT_R_auth_cycles.elapsed(), vePBFT_R_auth_cycles.max_increment(), num_vePBFT_R_auth);

  printf("\nPBFT_C_Reply MAC stats:\n");
  printf("Generate = %qd (cycles) %qd (max. op cycles) ops= %ld \n",
	 reply_auth_cycles.elapsed(), reply_auth_cycles.max_increment(), reply_auth);
  
  printf("Verify = %qd (cycles) %qd (max. op cycles) ops= %ld \n",
	 reply_auth_vePBFT_R_cycles.elapsed(), reply_auth_vePBFT_R_cycles.max_increment(), reply_auth_ver);
  ...
  printf("Verify = %qd (cycles) %qd (max. op cycles) ops= %ld \n",
	 sig_vePBFT_R_cycles.elapsed(), sig_vePBFT_R_cycles.max_increment(),num_sig_ver);
...
void PBFT_R_Statistics::init_rec_stats() {
  rec_counter++;
  ...
  rs.rPBFT_R_time = rPBFT_R_time.elapsed();
...
void PBFT_R_Statistics::end_rec_stats() {
  rec_time.stop();
  Recovery_stats& rs = rec_stats.high();
  ...
  rs.rPBFT_R_time = rPBFT_R_time.elapsed() - rs.rPBFT_R_time;
