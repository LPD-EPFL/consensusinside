...
template<class T> O_Certificate<T>::O_Certificate(int comp) :
...
template<class T> O_Certificate<T>::~O_Certificate()
{
}

template<class T> bool O_Certificate<T>::add(T *m, O_Node *n)
{
	const int id = m->id();
...
template<class T> bool O_Certificate<T>::add_mine(T *m, O_Node *n)
{
	const int id = m->id();
	...
	if (n->is_replica(id))
	{
		...
		fprintf(stderr,"O_Certificate.t: Replica %d: ", n->id());
