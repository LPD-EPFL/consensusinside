#ifndef _zl_Certificate_h
#define _zl_Certificate_h 1
...
template<class T> class zl_Certificate
{
   //
...
template<class T> inline T *zl_Certificate<T>::cvalue() const
{
   return value;
}

template<class T> inline bool zl_Certificate<T>::is_complete() const
{
   //fprintf(stderr, "gags.count = %d\n", count);
   return count >= complete;
}

template<class T> inline void zl_Certificate<T>::clear()
{
   count = 0;
   bmap.clear();
   value = 0;
...
template<class T> inline void zl_Certificate<T>::set_complete(int c)
{
  complete = c;
}

#endif // zl_Certificate_h
