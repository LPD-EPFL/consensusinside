#ifndef _T_Rep_info_h
#define _T_Rep_info_h 1
...
private:
   int nps;
   char *mem;
   T_Reply** reps; // Array of replies indexed by principal id.
   static const int Max_rep_size = 8192;
};

inline int T_Rep_info::size() const
{
   return (nps+1)*Max_rep_size;
}

inline Request_id T_Rep_info::req_id(int pid)
{
   return reps[pid]->request_id();
}

inline T_Reply* T_Rep_info::reply(int pid)
{
   return reps[pid];
}
