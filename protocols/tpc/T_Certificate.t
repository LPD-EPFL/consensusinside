...
template<class T> T_Certificate<T>::T_Certificate(int comp) :
...
template<class T> T_Certificate<T>::~T_Certificate()
{
}

template<class T> bool T_Certificate<T>::add(T *m, T_Node *n)
{
	const int id = m->id();
...
template<class T> bool T_Certificate<T>::add_mine(T *m, T_Node *n)
{
	const int id = m->id();
	...
	if (n->is_replica(id))
	{
		...
		fprintf(stderr,"T_Certificate.t: Replica %d: ", n->id());
