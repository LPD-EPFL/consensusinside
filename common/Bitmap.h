...
public:
  ...
  void
  assign(unsigned int i, bool value);
  ...
  bool
  operator==(Bitmap const other) const;
...
private:
  ...
  Chunk
  bitSelector(unsigned int i) const;
  ...
  Chunk
  valBitSelector(unsigned int i, bool val) const;
...
inline
Bitmap::Bitmap(Uint sz, bool value)
{
  num = sz;
...
inline Chunk
Bitmap::bitSelector(unsigned int i) const
{
  return 1UL << (i % ChunkBits);
}

inline Chunk
Bitmap::valBitSelector(unsigned int i, bool val) const
{
...
inline bool
Bitmap::test(unsigned int i) const
{
...
inline void
Bitmap::assign(unsigned int i, bool val)
{
  th_assert(i < num, "Index out of bounds\n");
  chunks[i / ChunkBits] &= (~bitSelector(i));
...
inline void
Bitmap::setRange(Uint min, Uint max)
{
...
inline void
Bitmap::resetRange(Uint min, Uint max)
{
  th_assert(min < num && max < num, "Index out of bounds\n");
  for (Uint i = min; i < max; i++)
    reset(i);
...
inline
Bitmap::Iter::Iter(Bitmap *b, bool v) :
  bitmap(b), value(v), index(0), ignoreValue((v) ? 0 : ~0)
{
}

inline bool
Bitmap::Iter::get(Uint& ind)
{
